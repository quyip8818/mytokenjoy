...
| 🔒 安全合规 | 模型配置（按团队控制可用模型） + 审计日志 + 敏感词审查   |
| 📊 可观测性 | 成本看板 + 用量分析 + 调用追踪       |
...
架构与计费详见 [Backend-架构.md](./Backend-架构.md)、[Backend-计费模式.md](./Backend-计费模式.md)、[Backend-预算.md](./Backend-预算.md)。
...
- **统计卡片区**：规则总数、已启用数、团队覆盖率、项目覆盖率
- **筛选工具栏**：类型 Tab（全部/团队/项目）+ 状态下拉（全部/启用/禁用）+ 搜索框
- **预警规则表格**：监控对象（含类型Badge）、阈值、触发动作（含图标）、通知角色（展示角色名+tooltip）、状态 Switch、操作 DropdownMenu
- **空状态**：引导性设计（图标 + 描述 + CTA 按钮）
- 禁用规则行降低透明度视觉弱化

**业务规则**：

- 预警规则可针对团队或项目单独配置
- 预警阈值可添加多个（预设 80%/90%/100% 或自定义）
- 触发动作：硬拒绝 / 审批追加 / 降级路由
- 通知角色：选择需要通知的角色
- 规则支持启用/禁用
- 通知发送失败不影响阻断逻辑
...
| #   | GIVEN                          | WHEN                                 | THEN                                 |
| --- | ------------------------------ | ------------------------------------ | ------------------------------------ |
| 1   | 超管进入预警规则页             | 添加阈值 80%、90%，选择通知角色，保存 | 配置保存成功，列表显示新规则        |
| 2   | 预警阈值 80%，张三额度 1000 元 | 张三累计消耗达 800 元                | 系统向通知角色发送预警               |
| 3   | 张三额度 1000 元，未获追加     | 消耗达 1000 元后再次调用             | 请求阻断（硬拒绝）                  |
| 4   | 成员邮箱不可达                 | 触发预警                             | 记录通知失败日志，不影响后续阻断逻辑 |
| 5   | 存在 3 条规则                  | 按"团队"类型筛选                    | 只显示监控对象为团队的规则           |
| 6   | 存在启用和禁用规则             | 按"已禁用"状态筛选                  | 只显示禁用的规则，行透明度降低       |
| 7   | 规则列表非空                   | 查看统计卡片                         | 显示正确的总数、启用数和覆盖率       |
...
| 场景 | 行为 |
|------|------|
| SaaS 版 | 仅展示内置模型列表，管理员可启用/禁用，无 Tab 切换，无"添加模型"按钮 |
| 私有化部署 | 展示全部模型（内置 + 自定义），支持 Tab 切换（全部/自定义/内置），支持添加自定义模型 |

**内置模型目录**（以国产模型为主）：

| 供应商 | 模型 | 特点 |
|--------|------|------|
| DeepSeek | V4, R1 | 超高性价比，推理能力强 |
| 通义千问 | 3.5 Plus, Max | 1M context，多模态 |
| 智谱 | GLM-5 | 中文理解领先，512K context |
| 月之暗面 | Kimi K3 | 1M 超长上下文旗舰 |
| 字节豆包 | Pro 256K | 256K 长上下文 |
| MiniMax | M2 | 多模态，性价比高 |
| Anthropic | Claude Sonnet 5 | 国际旗舰 |
| OpenAI | GPT-4o | 国际多模态 |
...
| 字段 | 必填 | 说明 |
|------|------|------|
| 模型名称 | ✅ | 模型唯一标识（callType） |
| 模型显示名称 | — | 界面展示名称 |
| API Key | ✅ | 认证密钥，支持明文/密文切换查看 |
| API endpoint URL | ✅ | 模型服务 Base URL |
| API endpoint中的模型名称 | — | 端点使用的模型标识 |
| 模型上下文长度 | ✅ | 默认 1,000,000（1M tokens） |
| 最大 token 上限 | — | 单次请求最大 token 数 |
| 输入单价 | — | ¥/M tokens |
| 输出单价 | — | ¥/M tokens |

**模型启用/禁用**：

- 所有模型（内置 + 自定义）均支持启用/禁用 Switch 开关
- 内置模型 toggle 通过"租户覆盖"实现（各租户独立控制，互不影响）
- 需要 `model:manage` 权限
- 操作进行中 Switch 禁用，防止重复请求

#### 9.2 模型配置（原模型白名单）

**目标**：为各团队配置可使用的模型名单，基于模型列表中已启用的模型进行选择。

**页面布局**：左侧组织树面板 + 右侧详情编辑区（split layout）

**右侧详情区**：

- 继承/自定义开关
- 按供应商分组的 checkbox 模型列表（全选/清空）
- 默认模型下拉选择
- 降级模型下拉选择
- 保存按钮
...
```mermaid
flowchart TD
    A[总公司<br/>全部 10 个模型] --> B[技术部<br/>自定义: 6/10]
    A --> C[产品部<br/>继承父级: 10]
    B --> D[后端组<br/>自定义: 4/6]
    B --> E[前端组<br/>继承父级: 6]
...
| #   | GIVEN                         | WHEN                                | THEN                                       |
| --- | ----------------------------- | ----------------------------------- | ------------------------------------------ |
| 1   | 超管进入模型列表页面（私有化）| 点击"添加模型"，填写信息含 API Key，保存 | 模型出现在自定义列表，状态启用         |
| 2   | SaaS 版管理员进入模型列表     | 查看页面                            | 无 Tab 切换、无"添加模型"按钮，只显示内置模型 |
| 3   | 管理员关闭一个内置模型的开关  | 操作完成                            | 该模型在本租户禁用，不影响其他租户     |
| 4   | 管理员重新打开已关闭的内置模型 | 操作完成                           | 该模型恢复启用                         |
| 5   | 总公司白名单 10 个模型        | 选择技术部，切换"自定义"，勾选 6 个 | 技术部白名单生效，显示"6/10 模型"      |
| 6   | 技术部白名单 6 个模型         | 配置后端组白名单                    | 只能从技术部 6 个中选                  |
| 7   | 技术部含 DeepSeek V4，后端组也选了 | 超管从技术部移除 DeepSeek V4    | 后端组白名单自动移除 DeepSeek V4       |
| 8   | 后端组白名单不含 Qwen Max     | 后端组成员调用 model=qwen-max-2026  | 返回错误"该模型不在您所属部门的可用范围内" |
| 9   | 成员发起 API 调用             | 请求中未包含 model 参数             | 返回错误"请指定模型"                   |
| 10  | 添加自定义模型时 API Key 为空 | 点击保存                            | 保存按钮禁用，无法提交                 |
