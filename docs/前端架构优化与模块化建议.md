...
├── routes/
│   └── {domain}/*.tsx  # 仅薄页面：{...useXxxPage()} + XxxPageShell
├── hooks/            # 跨域通用 hooks
└── lib/              # 跨域纯函数、常量
```

**已移除**：`routes/*/hooks/`、`routes/*/components/`、`components/{budget,org,keys}/`、`tests/routes/`。
...
| 项                           | 状态                                                                 |
| ---------------------------- | -------------------------------------------------------------------- | ------ |
| `routes` 仅薄入口            | 已完成：20 个注册页均 `import from '@/features/...'`                 |
| `routes/\*/hooks             | components` 遗留                                                     | 已删除 |
| `components/{domain}/` 遗留  | 已删除；领域 UI 仅在 `features/{domain}/components/`                 |
| `check-conventions.ts`       | `pnpm lint` 门禁；禁止 components 深导入 hooks、禁止 `tests/routes/` |
| PageShell / DataSection 统一 | 持续维护；新页遵循 `*-page-shell.tsx` 模式                           |

**新增代码规则**：业务逻辑与 UI 只进 `features/{domain}/`；跨域类型经 barrel（`@/features/keys`）或对方域 barrel 引用，组件不得 `from '@/features/x/hooks/'`。

### 2.2 业务域模块化（已完成主迁移）

**budget、org、keys、audit、dashboard、models、wallet、member** 均已迁入 `features/{domain}/`，各域含 `hooks/`、`components/`、`lib/`（如 `features/keys/lib/types.ts` 存放 `PlatformKeyTab` 等纯类型）。

后续可选：大 hook 拆分（`use-structure-page`、`use-budget-page` 等，见原计划 Phase 3）。
