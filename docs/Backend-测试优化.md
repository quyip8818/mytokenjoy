...
相关约定见 [Backend.md §5](./Backend.md#5-测试与-seed)。Gateway 拒绝场景可选优化：`tests/testutil/gateway/rejection_cases.go`（低优先级，可独立 PR）。
...
| N4 | worker 路由（可选） | 扩 `tests/worker/processors_test.go` | 1 |

#### C. `handler/core/mutating_contract_test.go`

| # | Case | 说明 |
| --- | --- | --- |
| H1 | PUT `/api/budget/departments/:departmentId` | 替代已删 `budget_test.go` |
| H2 | PUT approval reject（UUID） | 写操作 smoke |
...
company grants 落库链、memberanalytics 聚合、alert 通知、handler/keys 分页。
...
**PR3 可选（低优先级）：** `rejection_cases.go`、handler G2/G3 — 详见 **§12**。

| PR | 范围 | 状态 |
| --- | --- | --- |
| **PR1** Track S | test-fast 边界、删重复、newapiunits SSOT | ✅ |
| **PR2** Track C | middleware、outbox N1–N3、mutating H1–H2 | ✅ |
| **PR3** | Gateway rejection_cases + G2/G3（可选） | 部分完成；handler G2/G3 待做 |
| **permission 外迁** | `tests/infra/permission/` | ✅ |

---

## 12. PR3 实施规格

> 代码实现的唯一技术说明（可选 PR；plan.md §9 已关闭）。
