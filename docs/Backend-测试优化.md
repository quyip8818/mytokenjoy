...
相关约定见 [Backend.md §5](./Backend.md#5-测试与-seed)。
...
Gateway precheck / evaluate / handler 对同一拒绝场景有**重复 case 数据**（非功能盲区）。PR3 可选抽 `rejection_cases.go` + handler G2/G3 smoke；**完整 PR3 优先级低**（见 team 讨论）。
