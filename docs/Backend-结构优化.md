...
| 模式 | 定义 | 注入 | 说明 |
| --- | --- | --- | --- |
| Job enqueuer（5 域） | 各域 `ports.go` | `app/*_enqueuer.go` | budget / usage / dashboard / newapisync / org-remote |
...
| 缓存 / 通知 | 域内具名文件 | `wire_gateway.go` / `wire_river.go` / `wire_domain_services.go` | 如 `GatewaySoftCache`、`core.Notifier` |
...
| 1 | **低** | 文档 | [2.5 文档 stale 路径](#25-文档-stale-路径) | 部分已修 |
| 2 | **低** | 测试 | [2.6 internal 单测外迁](#26-internal-单测外迁) | 待做 |
...
**目标：** 其余 doc 中同类 stale 路径改为现行路径；历史描述用过去式或删段。
