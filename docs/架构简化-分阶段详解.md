...
`/v1` 是最高频路径：客户端每一次模型调用都会走预检。现状问题不是「余额读得慢」（`balance_point` 已经是 O(1)），而是：

1. **往返多**：mapping → company → 开账月 → remain → key → allowlist，Gateway 层多次散落 store 调用。
2. **热路径打 NewAPI**：`checkNewAPIWalletCap` / `checkWalletSyncLag` 会 HTTP 读 NewAPI quota；NewAPI 抖一下，合法请求也可能 **503**。
3. **职责混**：预检既信 Postgres，又信 NewAPI 派生配额，边界不清，难测、难维护。
...
- Phase 1 可以先基于现有四轴 snapshot 写 SQL；Phase 2 把轴减到两个后，SQL 再收紧。
...
## Phase 2 — Schema / Ingest：把写路径和表边界理顺

### 干什么

1. 保留钱包投影列（`balance_point` → rename `wallet_remain`），但**写入口收口**。
2. Ingest 热路径只维护 **两轴** snapshot（`platform_key` + `org_node`）。
3. 停写 `usage_buckets`；拆 `org_nodes` 胖表。

### 为什么要这么做

Gateway 读得再快，若 Ingest 每次写四轴 + hour bucket + 分散改余额，**写放大和维护成本**会拖垮迭代速度。
...
flowchart TB
  L[usage_ledger 追加]
  L --> S1[platform_key snapshot +]
  L --> S2[budget_group snapshot +]
  L --> S3[member snapshot +]
  L --> S4[org_node 祖先 rollup]
  L --> B[usage_buckets 小时桶]
  L --> LOT[lot 扣减]
  LOT --> BP[UPDATE balance_point]
```
...
| 现象 | 后果 |
| --- | --- |
| 四轴 snapshot | 预检其实只用企业钱包 + 部门 + Key；member/group 热写浪费 |
| `usage_buckets` 同步写 | 看板读路径与入账强耦合，失败/回放难 |
| `org_nodes` 胖表 | 组织树变更 vs 预算配置 vs 路由规则互相牵制 |
| 余额写分散 | `lot_allocate` / `billing_repo` 都能改 `balance_point`，投影易漂 |

### 例子：同一笔 12.5 point 消耗

**现状写放大（概念）：**

```text
ledger +1 行
snapshot platform_key  +12.5
snapshot member        +12.5   ← Phase 2 停写
snapshot budget_group  +12.5   ← Phase 2 停写（若挂组）
snapshot org_node 链   可能多行 +12.5
usage_buckets          upsert  ← Phase 2 停写
lot + balance_point    更新
```
...
ledger +1 行
snapshot platform_key  +12.5
snapshot org_node 链   +12.5
写路径 WalletService：同事务 UPDATE lot + wallet_remain
```

member / budget_group 的消耗？→ **冷路径**从 `usage_ledger` 聚合（看板、审计），不挡 Ingest。

### 钱包投影：为什么不删列

有人会问：既然 lot 是 SSOT，能不能删掉 `balance_point`，每次 `/v1` `SUM(points_remaining)`？

**不能。** `/v1` 极高频，`SUM(lot)` 会变成热路径聚合。正确做法：

```mermaid
flowchart LR
  LOT[company_recharge_lots<br/>SSOT] -->|同事务写穿| WR[wallet_remain<br/>投影列]
  WR -->|O1 读| GW[/v1]
  LOT -.->|冷路径对账| SUM["SUM(lot)"]
```

| 角色 | 对象 |
| --- | --- |
| 事实 | `company_recharge_lots` |
| 投影 | `companies.wallet_remain`（现 `balance_point`） |
| 唯一写手 | **写路径 WalletService**（充值 / Ingest 扣 lot / adjust） |

> 勿与现有 `domain/company.WalletService` 混淆：那是 **读 NewAPI quota 的 HTTP 客户端**。
...
```text
org_nodes
  id, parent_id, path, name     ← 组织结构
  budget, reserved_pool         ← 预算配置
  default_model_id, routing_*   ← 路由
```
...
```text
org_nodes          → 只要树
org_node_budget    → budget / reserved_pool / period
model_allowlist    → 路由（已有能力迁全）
```

改部门名不再碰预算行锁；改预算不再误触组织树约束。

### 具体交付清单

| 项 | 动作 |
| --- | --- |
| `wallet_remain` | rename；写收口到写路径 WalletService |
| `budget_snapshots` | 热路径只保留 `org_node` + `platform_key` |
| `usage_buckets` | 删表或改 MV；Ingest 停写 |
| `org_nodes` | 拆 `org_node_budget`；路由进 allowlist |
| seed / 单测 | wipe 重建；Gateway round-trip **不增加** |

### 验收直觉

同一笔消耗，DB 写次数明显下降；看板仍能给出 member 消耗（冷聚合）；`/v1` 仍 O(1) 读投影列。
...
sequenceDiagram
  ...
  Note over PG: ledger + 两轴 snapshot + WalletService
