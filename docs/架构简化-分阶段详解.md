...
| Key | `sk-acme-alice` |
| 钱包 | `companies.wallet_remain = 5_000` |
...
```text
PrecheckContext
├── WalletState   { company_status, wallet_remain, ... }
...
| **B** | `domain/wallet.Projection`：lot + 投影列唯一写入口 |

**已落地：** `org_node_budget` 表；`UpdateNode` / 审批 / 成员均预算写 `OrgNodeBudget()` 而非 `SetTree`；Ingest 与充值经 `wallet.Projection` 更新 `wallet_remain`；Gateway 读 `wallet_remain` 投影列。
