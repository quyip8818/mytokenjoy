...
`apps/backend/` Go 服务，实现 [Frontend-API契约.md](./Frontend-API契约.md) 全部 **81** 个管理面端点。种子数据在 `internal/seed/`；用量事实表 `usage_buckets` + webhook ingest。
...
| 模式 | 条件 | 说明 |
| ---- | ---- | ---- |
| Memory | `DATABASE_URL` 为空 | 启动加载 seed；适合本地与单测 |
| Postgres | 设置 `DATABASE_URL` | 域数据 JSONB snapshot + relay/usage/credential 关系表 |
...
cd apps/frontend && pnpm start   # .env.development 代理到 :8080
```

Dev：访问 `/login` 选成员 → cookie → 所有 `/api/*` 经 Vite 代理到 Go。

```
Browser → /api/* → apps/backend:8080
              ├─ Memory（默认）
              └─ Postgres + worker（可选）
```
...
- [ ] `apps/backend/internal/seed/`（若 demo 数据变）
