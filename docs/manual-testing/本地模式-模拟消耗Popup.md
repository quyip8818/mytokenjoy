...
| **catalog** | demo seed 含 `local-test-model`（ID 9）；旧库需 `pnpm docker:reset` 或重建后 reseed |
| **channel** | 模型 `local-test-model`，`base_url` → `http://host.docker.internal:8765/v1`，group 与 Key 部门一致（默认 `dept-dept-3`） |
| **Key** | 登录 `admin@example.com` / `demo1234`；Backend 启动时会为未 sync 的 seed Key 入队 NewAPI create（与线上一致）；Worker 跑完后 Popup 选 Key → `GET /api/dev/platform-keys/{id}/bearer` 只读拿 `sk-`（不 rotate）；白名单需含模型 9 |
