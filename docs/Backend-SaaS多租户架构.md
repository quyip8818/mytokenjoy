...
> **关联文档**：[TokenJoy-PRD.md](./TokenJoy-PRD.md) · [NewAPI-SaaS多企业配置.md](./NewAPI-SaaS多企业配置.md) · [Backend-设计.md](./Backend-设计.md) · [Backend-存储架构.md](./Backend-存储架构.md) · [Backend-命名规范.md](./Backend-命名规范.md)
...
| 企业钱包        | `companies.newapi_wallet_user_id` → NewAPI `users.quota` | 企业钱包用户 ID（NewAPI `users.id`），**非**成员 |
...
| 决策                 | 结论                                                                                 |
| -------------------- | ------------------------------------------------------------------------------------ |
| NewAPI 企业隔离      | 单集群；**每企业一个 `newapi_wallet_user_id`**；Token 绑 `user_id`；扣费进该企业钱包 |
| 计费主账             | **企业钱包** `users.quota`；充值只进钱包                                             |
| Token `remain_quota` | **分配视图**；`rebalance` 保证 Σ ≤ 钱包                                              |
| Relay Gateway        | 预检后透传 NewAPI                                                                    |
| 成员与钱包           | 成员**不**单独持有 NewAPI 账户；消费从 **企业钱包** 扣，按部门/Key 归因              |
...
- **企业**是付费与隔离边界：一个企业钱包、一套组织、多名成员。
- **成员**在企业内申请 Key、调 API；**资金**从企业钱包出；**花费配额**受部门 `budget` 约束（US-07 逐级分配）。
...
| ---------- | -------------------- | ---------- | ------------------------ | --------------------- | -------------------------------------- |
| **私有化** | `SUPPORT_SAAS=false` | 大企业自建 | 企业超管 `provider_keys` | `dept-{departmentId}` | 一个企业钱包用户                       |
| **SaaS**   | `SUPPORT_SAAS=true`  | 平台多客户 | 仅平台运营               | `platform_shared`     | **每企业**一个 `newapi_wallet_user_id` |
...
| `root_dept_id`          | 根部门                                                                |
| `newapi_wallet_user_id` | 企业钱包用户 ID（NewAPI `users.id`，非成员）                          |
...
1. 充值 → `TopUp(companies.newapi_wallet_user_id)` only。
...
```go
walletRemain := walletService.AvailableQuota(ctx, company.NewAPIWalletUserID)
...
req := newapi.CreateTokenRequest{
    UserID:      company.NewAPIWalletUserID,
...
| 方法           | 用途                             |
| -------------- | -------------------------------- |
| `CreateUser`   | 开户创建 `newapi_wallet_user_id` |
| `TopUp`        | 企业充值                         |
...
sequenceDiagram
    ...
    alt 预检失败
        ...
        GW->>NA: 原样透传（含 stream）
        NA->>NA: PreConsume + Settle（扣企业钱包用户 + Token）
        NA-->>GW: 响应
        GW-->>C: 原样返回
...
| 方法 | 路径                                    | 说明                                                      |
| ---- | --------------------------------------- | --------------------------------------------------------- |
| GET  | `/api/billing/wallet`                   | 企业超管/财务；`GetUserQuota` + 展示「可分配上限」        |
...
企业 Token 共用平台线路；**扣费仍进各自 `newapi_wallet_user_id` 对应钱包**。
...
| `internal/domain/usage/ingest.go`     | mapping `company_id`；按企业读写                         |
...
| 企业钱包 | 充值进 `users.quota`；调用扣钱包                           |
| 分配约束 | Σ Token ≤ 钱包；充值后 rebalance 上调                      |
| 未充值   | API 403；控制台可登录                                      |
| 企业隔离 | A 成员 Session 不能读 B 数据；Header 不能换企业            |
| NewAPI   | A 的 Token `user_id` = A 的 `newapi_wallet_user_id`        |
...
| 场景                                 | 文件                                                     |
| ------------------------------------ | -------------------------------------------------------- |
| Onboarding（开户→邀请→激活→session） | `tests/handler/onboarding_test.go`                       |
| Gateway 双轴预检                     | `tests/handler/gateway_test.go`                          |
| 企业隔离（org/budget/keys）          | `tests/handler/company_isolation_test.go`                |
| CreateCompany / invite               | `tests/domain/company/*_test.go`                         |
| Billing 钱包 / 充值                  | `tests/domain/billing/`、`tests/handler/billing_test.go` |
| Platform 登录 / 企业 CRUD            | `tests/handler/platform_test.go`                         |
| SaaS 路由契约                        | `tests/handler/contract_test.go`                         |
| Postgres SaaS 表                     | `tests/store/postgres/company_roundtrip_test.go`         |

### 11.3 反模式（明确禁止）

- 测试中依赖 `X-Company-Id` 切换租户（隔离只靠 session member → company）
...
SaaS 相关测试使用 `tests/testutil/saas.go`：
...
| `BuildGatewayScenario(...)` | Gateway 预检 fixture                                 |

Handler 测试入口：`newTestApp(t, ApplySaaSConfig)` + `StartNewAPIMock` 注入 `NewAPIBaseURL`。
...
| 问题                      | 答案                                                                  |
| ------------------------- | --------------------------------------------------------------------- |
| 余额在哪？                | 企业钱包：NewAPI `users.quota`（`companies.newapi_wallet_user_id`）   |
| 成员和 NewAPI user 区别？ | 成员登录 TokenJoy；`newapi_wallet_user_id` 仅扣费载体，非成员登录账号 |
