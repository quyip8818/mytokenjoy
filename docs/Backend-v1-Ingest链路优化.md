...
**Lag 与性能分开看**  
企业钱包（`wallet_remain`）和预算管控剩余（`combined_key_remain`）均在 ingest 事务里同步更新，预检读 PG **无投影 lag**。`budget_consumed` 同样在 Ingest 同事务写入（批量 UPSERT），用于 overrun 阻断和百分比预警。
...
`budget_consumed` 和 `combined_key_remain` 均在 Ingest 同事务内写入，无需 Projector。  
Advisory lock 仅用于 reconcile 冷路径和预算管理写入。
