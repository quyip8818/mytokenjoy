...
> **预算副作用**：[Backend-Projector.md](./Backend-Projector.md) · [Backend-budget_consumed迁回Ingest.md](./Backend-budget_consumed迁回Ingest.md)  
...
flowchart TB
  subgraph lineA [线 A — ingest.Worker]
    ...
    ING -->|InsertInTx 稀| DP[dashboard_project]
...
| `dashboard_project` | low | args，~1h | L0 ingest（稀）；L1 自续；L2 lag | `workers/dashboard_project.go` | `dashboard.Projector.RunBatch` |
...
2. `InsertDashboardProject`（稀 debounce）
3. `InsertWalletSync`
4. **可选** `InsertOverrun`（仅轻量预判认为可能触顶时）

**不**入队 `budget_projection`。rebalance 默认不在每笔 Ingest 入队（充值 / 月切 / reconcile / 按需）。

### 5.2 L1 — 业务路径（`Insert`）

| 来源 | kind | 说明 |
| --- | --- | --- |
| Ingest 预判触顶 | `overrun` | 可先判跳过；见 [Backend-Projector.md](./Backend-Projector.md) |
| 按需 / 充值 / Key 变更 | `rebalance` | 按轴 Unique |
| `schedule.EnsureMonthRebalance` | `rebalance`（company） | 月切；reconcile 批首或看门狗 |
| `billing.afterRecharge` | `wallet_sync`、`rebalance`（company） | 不阻断充值 |
...
| `dashboard.Projector` 批满 | `dashboard_project` | 自续 |
...
- 同事务仅稀唤醒 `dashboard_project` / `wallet_sync`；**无** `budget_projection`
- overrun：轻量预判后**按需**入队；百分比告警可直做
- rebalance：按需 / 充值 / 月切 / reconcile；非每笔伴随
- Gateway 读 `combined_key_remain`；看板读 `usage_buckets`
- 详见 [Backend-Projector.md](./Backend-Projector.md) · [Backend-budget_consumed迁回Ingest.md](./Backend-budget_consumed迁回Ingest.md)
