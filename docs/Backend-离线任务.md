...
| `rebalance` | default | per axis | L1 Projector 批末 / 充值月切 / L2 | `workers/rebalance.go` | `budget.Rebalancer.ProcessAxis` |
| `overrun` | default | per payload | L1 Projector 批末 | `workers/overrun.go` | `budget.OverrunProcessor` |
| `org_sync` | default | per company | L1 ScheduledAt；L2 看门狗补漏 | `workers/org_sync.go` | `org.RunScheduledSync` |
| `budget_projection` | default | args，1s 窗 | L0 ingest；L1 批满自续；L2 看门狗 | `workers/budget_projection.go` | `budget.Projector.RunBatch` |
| `budget_reconcile` | low | args，**24h** | L1 手动 API；L2 看门狗 | `workers/budget_reconcile.go` | `budget.ReconcileService.RunCompany` |
...
| `budget.Projector.RunBatch` 批末 | `rebalance`、`overrun` | 批末扇出；方向：轻量副作用可直做，判断轻则可先判再入队（[Backend-Projector.md](./Backend-Projector.md)） |
...
- Ingest 写 `usage_ledger` + lot；同事务入队 `budget_projection` + `dashboard_project` + `wallet_sync`（**不**在热路径执行 overrun/rebalance）
- `budget.Projector`：写 `budget_consumed` / `combined_key_remain`；批末扇出 `rebalance` / `overrun`；百分比告警等轻量检查可批内直做
- **方向**：轻则直做；判断轻则可先判再决定是否 fanout；重则 River debounce（`ByPeriod` / `ByArgs`）——见 [Backend-Projector.md](./Backend-Projector.md)
- Gateway 预检读 `combined_key_remain`（经 `budgetcheck` 缓存）
- 看板读 `usage_buckets`（`dashboard.Projector`）
- consumed / combined 迁回 Ingest 的目标见 [Backend-budget_consumed迁回Ingest.md](./Backend-budget_consumed迁回Ingest.md)
