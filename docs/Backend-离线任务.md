...
> **待办**（`budget_project`、Periodic fanout）：[实现-离线任务管理.md](./实现-离线任务管理.md)  
...
flowchart TB
  subgraph lineA [线 A — ingest.Worker]
    ...
    ING --> LEDGER[usage_ledger + budget_snapshots Apply]
    ING -->|InsertInTx| RJ[river_job]
    DRIFT[ReconcileWalletDrift] -->|Insert| WSjob[wallet_sync]
  end
...
| `rebalance` | default | per axis | 入账 side effect、充值、月切 | `workers/rebalance.go` | `budget.Rebalancer.Run` |
| `overrun` | default | per payload | 入账 side effect | `workers/overrun.go` | `budget.OverrunProcessor.Run` |
| `org_sync` | default | 无 | Periodic | `workers/org_sync.go` | `org.SyncService.RunScheduledSyncAll` |
| `monthly_rebalance` | default | 无 | Periodic | `workers/monthly_rebalance.go` | `MonthlyRebalanceScheduler.EnqueueMonthlyRebalanceAll` |

Args 与 `InsertOpts()`：`internal/infra/jobs/args.go`。  
入队 helper：`internal/infra/jobs/enqueue.go`。

**尚未实现**（规划见 [实现-离线任务管理.md](./实现-离线任务管理.md)）：`budget_project`、`budget_reconcile`、`dashboard_project`、`dashboard_reconcile`。
...
1. `ledger` 写入、`Apply` 更新 `budget_snapshots`
2. `enqueueSideEffects` → `rebalance`（member / dept / budget_group）+ `overrun`
3. `InsertWalletSync`

任一步失败 → 整笔事务回滚（含已插入的 `river_job` 行）。

`enqueueSideEffects`：`domain/usage/side_effects.go`（要求 `store.Tx` 非 nil）。
...
| River Periodic | `org_sync`、`monthly_rebalance` | 见 §6 |
...
- 行为与迁移前 processor 等价；测试见 `tests/worker/processors_test.go`
...
- 单条 Periodic job → `RunScheduledSyncAll` → `company.ForEachActiveCompany` → 每 tenant `RunScheduledSync`
...
| --- | --- | --- | --- |
| 组织同步 | `WORKER_ORG_SYNC_INTERVAL_SEC` | 60s | `org_sync` |
...
- Ingest 仍在事务内 **同步** `Apply` 写 `budget_snapshots`
- 同事务入队 `rebalance` / `overrun` / `wallet_sync`（非 `budget_project`）
- Gateway 预检仍读 snapshot / 同步投影路径

目标态（ledger 单写 + `budget_project` + 批末 side effect）见 [实现-异步预算投影.md](./实现-异步预算投影.md)、[Backend-预算.md](./Backend-预算.md)。
...
- 保留 `scheduler_locks`：仅 Ingest reconcile，与 River leader 无关
...
internal/
  ...
  config/river.go              # RIVER_* env
  domain/usage/ingest.go       # 事务内 side effect + wallet_sync
  domain/usage/side_effects.go
  domain/billing/wallet_sync.go
  domain/billing/lot_confirm.go # afterRecharge
  domain/budget/monthly_rebalance.go
  domain/org/remote/sync_all.go
...
| [实现-离线任务管理.md](./实现-离线任务管理.md) | 剩余 kind 与验收 |
| [实现-异步预算投影.md](./实现-异步预算投影.md) | `budget_project` 与 Ingest 目标态 |
