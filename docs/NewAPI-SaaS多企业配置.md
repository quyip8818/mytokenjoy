...
> **约定**：产品称 **企业（Renter）**；NewAPI 里的 `users` 在本方案中仅作 **企业服务账户（公司钱包）**，不是企业员工登录账号。
...
| TokenJoy Backend | 1+ | `MULTI_TENANT=true`；唯一持有 NewAPI Admin Token |
...
| **logs** | webhook 回传 `token_id` | TokenJoy 映射表反查 `renter_id` |
...
平台 `POST /api/platform/renters`（或等价）时，Backend 依次：

| 步骤 | NewAPI Admin API（概念） | 结果 |
|------|--------------------------|------|
| 1 | `CreateUser` | 企业服务账户；`quota = 0`；`username` 建议 `renter-{id}` 或企业 slug |
| 2 | — | 将返回的 `user_id` 写入 `tenants.newapi_wallet_account_id` |
...
- [Backend-SaaS多租户改造.md](./Backend-SaaS多租户改造.md) — 后端实现与 Store  
