...
Go 后端通过环境变量 `APP_PROFILE` 区分运行模式（默认 `demo`）：

| Profile | 值     | GET 读接口                           | Session 身份解析                                               |
| ------- | ------ | ------------------------------------ | -------------------------------------------------------------- |
| Demo    | `demo` | 不要求 Session（开发联调便利）       | 允许 `memberId` query、`X-Demo-Member-Id`、cookie、Bearer      |
| 生产    | `prod` | 要求 Session + 对应读权限（见 §3.4） | 仅 cookie `tokenjoy_session_member` 或 `Authorization: Bearer` |

写操作（POST / PUT / PATCH / DELETE）在两种 Profile 下均要求 Session + 写权限。本地联调默认 `demo`；部署生产应设置 `APP_PROFILE=prod`。

### 3.3 其他端点

Mock 层**不做鉴权**（任意请求可通过）。权限由 `GET /session` 返回的 `permissions[]` 驱动，前端通过 `PermissionGate` / `usePermissions()` 控制 UI 与路由。

真实后端（`APP_PROFILE=prod`）对读、写操作均按 permission key 校验：未登录 → `401`；已登录但无权限 → `403`。Demo Profile 下 GET 读接口不校验 Session，写接口仍校验。
