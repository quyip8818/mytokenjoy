...
| `Cookie`        | Session    | `credentials: 'include'` 自动携带；dev 登录写入 `tokenjoy_session_member` |
...
| 变量                    | 说明                                                           |
| ----------------------- | -------------------------------------------------------------- |
| `VITE_API_PROXY_TARGET` | 本地开发将 `/api` 代理到 Go 后端（如 `http://localhost:8080`） |
| `BASE_URL`              | Vite 子路径部署前缀；影响 `API_BASE_PATH`                      |

根目录 `pnpm start` 并发启动 backend + frontend；[`apps/frontend/.env.development`](../apps/frontend/.env.development) 默认配置代理目标。

### 8.2 鉴权

- Dev：访问 [`/login`](../apps/frontend/src/routes/auth/login.tsx) 选择成员，写入 `tokenjoy_session_member` cookie
- 生产：网关或后端签发 `Authorization: Bearer` token
- 401 时 `AuthUnauthorizedBridge` 跳转登录页
