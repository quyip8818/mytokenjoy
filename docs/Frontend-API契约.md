...
| `Cookie`        | Session    | `credentials: 'include'` 自动携带；企业面 `tokenjoy_session_member`；平台面 `tokenjoy_platform_session`（SaaS 规划，见 §10.2） |
...
### 3.5 SaaS 双 Session（规划）

| 面 | Cookie | 登录入口 | 说明 |
| -- | ------ | -------- | ---- |
| 企业面 | `tokenjoy_session_member` | `/login` 或邀请激活 | `GET /session`；`tenantId` 来自 Session，**不可**用 Header 覆盖 |
| 平台面 | `tokenjoy_platform_session` | `/platform/login`（前端路由待定） | `POST /platform/auth/login`；与企业 Session 互不影响 |
...
| `audit:read`      | 审计日志                                        |
| `api:call`        | API 调用权限                                    |
| `billing:read`    | **SaaS 规划**：公司钱包只读                      |
| `billing:recharge`| **SaaS 规划**：企业自助充值                      |
| `platform:*`      | **SaaS 规划**：平台运营（独立 Session，不走成员权限表） |
...
| `tenantId`    | `number`   | **SaaS 规划**：当前成员所属企业 ID；私有化默认 `1` |
...
- **平台面（SaaS 规划）**：`POST /platform/auth/login` → `tokenjoy_platform_session`；与企业面分离
- **邀请激活（SaaS 规划）**：`POST /auth/accept-invite` 设密并创建成员 Session
...
- [ ] 本文档 §5 端点清单（SaaS 增量见 §10）
- [ ] 后端 handler + domain service（[`apps/backend/`](../apps/backend/)）
- [ ] 页面 Hook / 测试 — 按需补充

---

## 10. SaaS 多企业（规划，未纳入 81 端点）

产品模型：**企业（Renter）** = 一家公司；**成员（User）** = 企业内员工。计费双轴（公司钱包 + 部门 budget）、Gateway、平台鉴权详见 [Backend-SaaS多租户改造.md](./Backend-SaaS多租户改造.md)。

**增量端点：共 10 个**（路径均相对于 `API_BASE_PATH`）。实现阶段见后端文档 §十（SaaS-1～4）。
...
无需 Session；成功后与 `GET /session` 结构一致（含 `tenantId`）。
...
客户端（规划）：`platformApi`（或 `platformAuthApi`）
...
其余 `/platform/*` 须携带平台 Session；`MULTI_TENANT=false` 时返回 404。

### 10.4 平台面：企业与 Channel

| 方法 | 路径 | Body / 查询 | 响应 | 说明 |
| ---- | ---- | ----------- | ---- | ---- |
| GET | `/platform/renters` | `page?`, `pageSize?`, `status?` | `Paginated<Renter>` | 企业列表 |
| POST | `/platform/renters` | `CreateRenterRequest` | `Renter` | 开户 + 发超管邀请 |
| PATCH | `/platform/renters/:id` | `PatchRenterRequest` | `Renter` | 状态、套餐 |
| POST | `/platform/renters/:id/recharge` | `{ amount }` | `RechargeOrder` | 平台代充；`source=platform` |
...
**`CreateRenterRequest`：** `name`, `slug`, `superAdminEmail`, `packageId?`

**`PatchRenterRequest`：** `status?`, `packageId?`, `name?`

> 路径以 `renters` 为准；不实现 `/platform/tenants`。

### 10.5 既有端点的 SaaS 行为差异

| 端点 / 域 | 私有化 | SaaS |
| --------- | ------ | ---- |
| `GET /session` | `tenantId = 1` | `tenantId` = 成员所属企业 |
| `provider-keys` 写 | 允许 | **403** |
| `POST /keys/platform` | 不变 | 不变；后端 Token 绑企业服务账户 + `platform_shared` |
| 组织 / 预算 / 密钥读写在企业面 | 单租户 | 按 Session `tenantId` 隔离 |
| 成员邀请（企业内） | 可选 | **规划**：`POST /org/members/invite`（契约待 SaaS-1 细化） |
...
- [ ] `SessionContextSchema` 校验 `tenantId`
