...
| `Cookie`        | Session    | `credentials: 'include'` 自动携带；企业面 `tokenjoy_session_member`；平台面 `tokenjoy_platform_session`（`MULTI_COMPANY=true`，见 §10） |
...
| 平台面 | `tokenjoy_platform_session` | `POST /platform/auth/login` | 与企业 Session 互不影响；`MULTI_COMPANY=false` 时平台路由 404    |
...
其余 `/platform/*` 须携带平台 Session；`MULTI_COMPANY=false` 时返回 404。
