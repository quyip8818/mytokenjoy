...
- 单进程 Worker + River `river_job`（规模未到再拆池，不必拆服务）；见 [实现-离线任务管理.md](./实现-离线任务管理.md)
...
2. **统一为 DB 事务 + River 入队**：任何 PlatformKey 变更 = `platform_keys` + `river_job(newapi_sync)` 同一事务（`InsertTx`）；见 [实现-离线任务管理.md](./实现-离线任务管理.md)
...
| 5 张 queue 表 | 已收口为 River `river_job` + kind；见 [实现-离线任务管理.md](./实现-离线任务管理.md) |
