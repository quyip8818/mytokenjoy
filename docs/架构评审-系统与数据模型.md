...
- Ingest：ledger INSERT + lot 扣减 + 同事务入队 `budget_project`；consumed 由 `budget.Projector` 异步写
...
- Ingest 写 `usage_ledger` + lot 扣减 + 入队 `budget_project`；`budget.Projector` 异步写 `budget_consumed` + `gateway_soft_*`
...
sequenceDiagram
  ...
  alt 通过
    GW->>NA: 转发
    NA-->>ING: settle log
    ING->>PG: ledger + 入队 budget_project
