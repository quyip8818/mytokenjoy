...
flowchart LR
  ...
  BS[budget_consumed 组织 consumed]
  LOT[company_recharge_lots 钱包]

  UL -->|异步 budget_projection| BS
  UL -->|异步 dashboard_project| UB[usage_buckets]
  UL -->|FIFO 扣减| LOT
...
| 组织预算 | `budget_consumed`（三轴 × `period_key`） | Gateway 预检（`gateway_soft_*`）、预算树 limit；部门花费见 `usage_ledger` |
| 企业钱包 | `company_recharge_lots` + `wallet_remain` | NewAPI `users.quota` |

**应保留的架构决策（勿动方向）：**

- `/api` 与 `/v1` 分轨；Gateway OpenAI 兼容反代
- `adminport.Port` 隔离 NewAPI Admin
- consumed 不进 `org_nodes` / `platform_keys` 列，统一 `budget_consumed`
- `platform_keys` 与 `platform_key_mappings` 拆分
- Ingest：ledger INSERT + lot 扣减 + 同事务入队 `budget_projection`；consumed 由 `budget.Projector` 异步写
...
1. **事实可重建** — `usage_ledger` / `company_recharge_lots` 为唯一真相；`budget_consumed`、buckets、NewAPI quota 均可从事实 **全量或分段重算**
...
- Ingest 写 `usage_ledger` + lot 扣减 + 入队 `budget_projection`；`budget.Projector` 异步写 `budget_consumed` + `gateway_soft_*`
- `budget_reconcile` / `dashboard_reconcile` 定期对比 ledger 与投影，修复 drift
- 钱包靠 `wallet_sync` 把 `wallet_remain` 推到 NewAPI quota；预算靠 `rebalance` 推 Key quota
- 已有 **ingest 补洞**（`reconcile_cursors`）和 **wallet 漂移 ε 对账**（[Backend-计费模式.md](./Backend-计费模式.md) §10）
...
| 步骤 | 交付物 |
| --- | --- |
| 1 | `budget.Projector` + `budget_reconcile`：按 ledger 聚合写三轴 consumed（已实现；可 TRUNCATE 后全量重跑） |
| 2 | `dashboard.Projector` + `dashboard_reconcile`：重建 `usage_buckets` |
| 3 | `domain/billing.RebuildBalancePoint(company_id)`：从 lots + ledger 闭合（待产品化运维入口） |
| 4 | 定期 **抽样对账**：`Σ ledger.amount` vs `budget_consumed` vs `usage_buckets` 超 ε 告警（reconcile job 已覆盖 per-company） |
...
> **与当前 as-built 的关系：** 上线前本地 / demo 仍可按 [Backend.md](./Backend.md) §2.5 **wipe + `schema.sql`** 演进；本节针对**有租户数据后**必须引入 incremental migration 的场景。
...
1. `companies.wallet_remain`
2. `platform_keys.gateway_soft_remain` + 各轴 limit（单 SQL `LoadPrecheckContext`）
3. 模型白名单 / Key 状态

**不参与预检：** NewAPI Key `remain_quota`、企业钱包 quota、`wallet_sync` 滞后。实现见 [Backend-架构.md](./Backend-架构.md) §6。

Ingest 写 ledger 后，`budget.Projector` 异步刷新 consumed 与 gateway 软缓存。运维契约：`wallet_sync` / `budget_reconcile` 负责消化漂移与 lag。
...
sequenceDiagram
  ...
  C->>GW: 请求
  GW->>PG: wallet_remain + gateway_soft
  GW->>NA: 转发（不读 quota）
  alt 通过
    GW->>NA: 转发
    NA-->>ING: settle log
    ING->>PG: ledger + 入队 budget_projection
...
1. **文档 + 代码层** 明确「硬门禁」为 **Postgres 事实**（`wallet_remain`、`gateway_soft_*`）与 **Key 有效性**；NewAPI quota 为 **执行面派生**
2. 统一 `domain/convergence`（或扩展现有 billing/usage）：`DriftReport(company_id)` → PG vs NewAPI vs 投影差值
3. Gateway 预检与 Ingest 共享 **同一套 Remain 计算**（`GatewayChainRemain` / `ComputeRemainForMapping` in `pkg/budget/mapping_remain.go`，经 `gateway_soft_*` 投影到预检）
...
| 后 | 预算配置迁出 → `org_node_budget(company_id, node_id, period_key)`，与 `budget_consumed.consumed` 对称 |
...
- **开账月** `OpenBudgetPeriod` → 预检、overrun、`budget_consumed` 读写
...
| 预算 | 三轴 snapshot + ledger 部门聚合 | Project limit | 简单 budget | 编制 + 发生 |
...
gantt
  ...
  wallet_remain 重算 + 对账 API  :p01b, after p01, 3w
