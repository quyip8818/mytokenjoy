...
> **关联**：[实现-架构终态.md](./实现-架构终态.md)（文件级实施清单）· [Backend-架构.md](./Backend-架构.md) · [Backend-预算.md](./Backend-预算.md) · [实现-异步预算投影.md](./实现-异步预算投影.md) · [Backend-离线任务.md](./Backend-离线任务.md)
...
| P2 | **事实单写** | 消耗只追加 `usage_ledger`；钱包只经 `domain/wallet` 更新 `wallet_remain` |
...
`gateway_soft_remain`：四轴 **最紧 remain**（与 `ComputeRemainBudget` 一致，Projector 批末写入）。
...
事务内：**ledger + wallet + `InsertTx(budget_project)` + `InsertTx(platform_sync)`**。
...
→ chain budget_project if full batch
```
...
| 预算树 / 成员额度变更 | `rebalance` | 对应 axis | 管理面 `EnqueueRebalanceAxis` |
...
| `projection_queue_depth` | `river_job` kind=`budget_project` pending |
...
| 故障 | 现象 | 恢复 |
| --- | --- | --- |
| Projector 卡住 | lag critical；queue 堆积 | 查 `budget_projection_progress`；手动 `Insert(budget_project)`；必要时 chain |
...
default:   budget_project, platform_sync, rebalance, overrun, org_sync
...
[实现-架构终态.md](./实现-架构终态.md) · [Backend-架构.md](./Backend-架构.md) · [Backend-预算.md](./Backend-预算.md) · [实现-异步预算投影.md](./实现-异步预算投影.md) · [Backend-离线任务.md](./Backend-离线任务.md) · [Roadmap.md](./Roadmap.md)
