...
`gateway_soft_remain`：参与预检各轴 **最紧 remain** 的 min（Projector 批末写入）。`member` = `min(key, personal, wallet)`；`project_member` 含 sub 与 project，见 [Platform-Key产品设计.md](./Platform-Key产品设计.md) §1 · `chain.go`。
