...
`gateway_soft_remain`：参与预检各轴 **最紧 remain** 的 min（Projector 批末写入）。`member` = `min(key, personal, wallet)`（**不含**未分配/预留池/部门报表）；`project_member` 含 sub（Key 聚合）与 project，见 [Platform-Key产品设计.md](./Platform-Key产品设计.md) §4。
