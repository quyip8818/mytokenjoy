...
│    充值 → lot（FIFO） → wallet_remain_quota                   │
...
flowchart TB
  ...
  subgraph pg ["Postgres（SSOT）"]
    WAL[(wallet_remain_quota<br/>+ lots FIFO)]
    ONB[(org_node_budget)]
...
flowchart TD
  ...
  C1 -->|blocked| R1[403]
  C1 -->|正常| C2{wallet_remain_quota >= 1?}
...
NewAPI logs.quota ──直接透传──► entry.Amount
                    ...
                    ┌───────────────┼───────────────┐
                    ▼               ▼               ▼
             wallet_remain_quota    budget_consumed   combined_key_remain
