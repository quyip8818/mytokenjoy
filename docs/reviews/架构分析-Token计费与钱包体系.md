...
| **Point** | 内部统一货币单位 | `wallet_remain`, `budget_consumed`, `ledger.amount` | 所有业务逻辑的计算基础 |
...
充值只增加 `wallet_remain`（企业硬顶），不自动增加 `org_nodes.budget`。
...
flowchart TB
  subgraph wallet_axis [轴 1：企业钱包（资金硬顶）]
    CHARGE[充值 → lot] --> WAL[wallet_remain]
...
|------|--------|-------------------|----------|
| `wallet_remain` | **Strong** | 是（L0 hard） | 充值/lot |
...
```mermaid
flowchart LR
  L0[L0 钱包硬顶<br/>wallet_remain<br/>无 lag] --> L1[L1 预算软限<br/>gateway_soft_remain<br/>≤ Projection Lag]
  L1 --> L2[L2 超限封禁<br/>Overrun Disable Key<br/>≥ L1 lag]
```

| 层级 | 机制 | 数据 | 超卖窗口 |
|------|------|------|----------|
| L0 | `wallet_remain >= ε` | 强一致 | 无 |
...
| `companies.wallet_remain` | 企业钱包余额 | Strong |
