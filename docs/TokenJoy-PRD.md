...
```mermaid
flowchart TD
    T[Tenant 总预算<br/>¥50,000/月] --> D1[技术部<br/>¥20,000]
    T --> D2[产品部<br/>¥15,000]
    T --> D3[市场部<br/>¥10,000]
    T --> R0[未分配<br/>¥5,000]
...
本文档记录 PRD User Story（US-01～US-14）与当前前端实现 / [Frontend-API契约.md](./Frontend-API契约.md) 之间的**剩余**差异。实现基准：`apps/frontend/src/api/`、`api/types/`、MSW handlers。**不含 US-15（合规审查）**。

**状态图例：** ➕ 超前（契约/实现已有，PRD 未单独描述）；➖ 范围外（PRD 提及，但不应由前端管理 API 承担）。

**最近对齐（2026-06-25）：** US-07 成员额度与 BG Key、US-11 `modelWhitelist`、US-13 成本看板（环比 / 自定义日期 / 趋势粒度）、US-14 审计筛选（时间 / 操作人 / 关键词）均已写入契约与 Mock。

### 剩余差距总览

US-01～US-14 与契约**已对齐**（除下表）。

| 标记 | 主题                 | 摘要                                                                                                      |
| ---- | -------------------- | --------------------------------------------------------------------------------------------------------- |
| ➕   | 供应商密钥等超前能力 | `providerKeyApi`、`defaultModel` / `fallbackModel`、`revoke`、用量看板等 — 本附录「实现扩展」可继续补全 |
...
出现新的 PRD 变更或契约扩展时，先更新 [Frontend-API契约.md](./Frontend-API契约.md) 与实现，再回本附录维护「剩余差距」。
