...
- 无需审批，在个人额度范围内自主创建
- 创建时选择绑定模型（不超出部门白名单）+ 分配额度（不超出个人剩余）
...
**目标**：定时扫描 API 调用输入内容，标记命中敏感词的记录。

**审查机制**：

```mermaid
flowchart TD
    A[API 请求正常放行] --> B[调用记录存储]
    B --> C[定时批量扫描<br/>仅扫描输入 prompt]
    C --> D{命中规则?}
    D -->|否| E[正常]
    D -->|是| F[标记违规<br/>状态: 待处理]
    F --> G{超管处理}
    G -->|确认| H[已确认违规]
    G -->|忽略| I[已忽略]
```
...
| 类型           | 说明                         | 操作                  |
| -------------- | ---------------------------- | --------------------- |
| 系统预设规则   | 政治敏感、暴力色情、个人隐私 | 可启用/停用，不可删除 |
| 企业自定义规则 | 自定义敏感词列表             | 可增删改              |
...
| #   | GIVEN                             | WHEN                               | THEN                                       |
| --- | --------------------------------- | ---------------------------------- | ------------------------------------------ |
| 1   | "政治敏感"规则已启用              | 定时扫描发现调用输入命中敏感词     | 记录出现在违规列表，状态"待处理"           |
| 2   | 自定义规则"商业机密"含"项目代号X" | 扫描发现输入包含"项目代号X"        | 记录被标记，命中规则显示"商业机密"         |
| 3   | 违规列表有一条待处理              | 超管点击"确认"                     | 状态变为"已确认违规"                       |
| 4   | 违规列表有一条待处理              | 超管点击"忽略"                     | 状态变为"已忽略"                           |
| 5   | 超管进入规则管理                  | 添加规则"竞品信息"，填敏感词，保存 | 规则创建成功，后续扫描使用该规则           |
| 6   | "个人隐私"规则启用                | 切换为停用                         | 后续扫描不再使用该规则                     |
| 7   | 内容留存开关关闭                  | 进入合规审查页面                   | 提示"请先开启内容留存才能使用合规审查功能" |
