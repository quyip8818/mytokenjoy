...
- **SaaS 服务**：平台托管多家 **企业（Renter）**；企业内有多名 **成员（User）**；数据按企业行级隔离
- **私有化部署**：同一制品，`MULTI_TENANT=false`，隐含一家企业

架构与计费详见 [Backend-SaaS多租户改造.md](./Backend-SaaS多租户改造.md)。
...
- 一家企业只能接入一个第三方平台（凭证按 `tenant_id` 隔离）
- 保存前必须通过「测试连接」验证
...
管理面 REST（81 端点）以 [Frontend-API契约.md](./Frontend-API契约.md) 为准；前后端均已对接。**不含 US-15（合规审查）**。
...
**契约不覆盖（由其他系统承担）：** 企业开户与邀请、LLM Relay Gateway（US-12）、公司钱包充值、IM/邮件通知、审计归档基础设施、服务端定时 sync cron。SaaS 增量 API 见 [Backend-SaaS多租户改造.md](./Backend-SaaS多租户改造.md) §八。
