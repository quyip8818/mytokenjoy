...
| 轴           | 权威数据源                                                         | 管什么                      | 谁改                                      |
| ------------ | ------------------------------------------------------------------ | --------------------------- | ----------------------------------------- |
| **企业钱包** | `company_recharge_lots` / `companies.balance_point`                | 预付资金硬上限（point）     | 充值确认写 lot → 异步同步 NewAPI          |
| **组织预算** | Postgres `org_nodes` 等 + `budget_snapshots`（point）               | 部门树额度、成员/Key/组花费 | 控制台写 limit；Ingest 累加 `consumed`    |

```mermaid
flowchart TB
  subgraph wallet [企业钱包轴]
    RCH[company_recharge_orders] --> LOT[company_recharge_lots]
    LOT --> BAL[companies.balance_point]
    BAL -.->|派生同步| NA_W[NewAPI users.quota]
  end

  subgraph org [组织预算轴]
    BT[org_nodes 树 budget=point] --> MEM[members.personal_quota]
    BT --> BG[budget_groups]
  ...
  BAL -->|rebalance 封顶| PK
  BT -->|ComputeRemainQuota point| PK
  ING[Webhook Ingest] -->|FIFO 扣 lot + 累加 consumed| org
  ING --> UB[usage_buckets 看板 point]
```

**充值只涨钱包 lot，不自动涨部门 `budget`。** 超管在控制台分配 `org_nodes.budget`（point）；成员额度、Key 配额、预算组额度从组织预算轴往下切。
...
| ----------------------------------- | ------------------------------------------- |
| 企业 `active`                       | `companies`                                 |
| 企业钱包 ≥ 预估                     | `companies.balance_point`                   |
| 部门 `consumed + estimate ≤ budget` | `budget_snapshots` + `org_nodes.budget`     |
| Token `remain_quota` 足够           | NewAPI Token                                |
| NewAPI 企业 `users.quota` 足够      | NewAPI（通道硬顶；`wallet_sync` 滞后超阈拒绝 proxy） |
...
NewAPI 每个 Token 有 `remain_quota`。以下时机把 **point「还能花多少」** 换算并 `UpdateToken`：

- Ingest 后（花费变化）
- 充值后（`balance_point` 变大）
- Key 创建/更新/启用
...
再经 `ToQuotaUnits` 换成 NewAPI 单位；封顶以 **Postgres `balance_point`** 为准（**Σ Token remain_quota ≤ 企业钱包 point 对应 units**）。NewAPI `users.quota` 仅作派生缓存。

```mermaid
flowchart LR
  Q[rebalance_queue] --> RB[RebalanceService]
  RB --> READ[读 tree / keys / balance_point]
  RB --> CALC[ComputeRemainQuota]
...
sequenceDiagram
  ...
  U->>TJ: ConfirmPayment
  TJ->>PG: INSERT lot + balance_point；order confirmed
  TJ->>PG: outbox wallet_sync
  TJ->>NA: 派生 TopUp / SetUserQuota
...
扣减 point 与展示币始终进 **Postgres lot FIFO**；NewAPI 仅结算通道并异步与 `balance_point` 校准。TokenJoy 用 `department_id` / `member_id` / `budget_group_id` 做归因与配额约束。
...
| Relay 可用 point    | `min(...)` → `ComputeRemainQuota`（point）                          |
| NewAPI 单位         | `points / 最贵模型单价 * QuotaPerUnit` → `ToQuotaUnits`             |
...
| `relay/quota.go`      | `ComputeRemainQuota`（point → units） |
