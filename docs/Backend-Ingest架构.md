...
TokenJoy **不自己跑模型**。真正转发请求、结算配额的是 **NewAPI（Relay）**。  
...
flowchart TB
  ...
  NA[NewAPI Relay<br/>转发上游 LLM + 结算]
...
flowchart LR
  subgraph admin [管理面 · Admin API]
    LC[TokenLifecycle] -->|HTTP Admin| NA1[NewAPI Token / User / Channel]
  end
...
| 对齐键 | `relay_mappings.newapi_token_id` ↔ NewAPI token 主键 |

**为什么 Rotate 不能 delete+create？**  
Ingest 靠 `logs.token_id` 反查 mapping。若 Rotate 换了 token 主键，旧日志对不上新 Key，入账归因断裂。因此 Rotate 走 regenerate，**保持 `newapi_token_id` 不变**。
...
```mermaid
flowchart LR
  Log["logs.token_id"] --> Map["relay_mappings<br/>newapi_token_id"]
  Map --> Co[company_id]
...
2. `FindMappingByNewAPITokenID` 找到映射；
...
```text
relay outbox → ingest_jobs 消费（webhook 队列 + 重试）→ reconcile 补洞 → rebalance → overrun → org sync …
```
...
flowchart TB
  ...
  API <-->|Remote-first Token 生命周期| ADM
  SDK --> GW --> RELAY --> SETTLE --> LOGS
...
| 配置 | 作用 |
| --- | --- |
| `NEW_API_ENABLED` | 打开 Relay 集成总开关 |
...
| `RELAY_GATEWAY_ENABLED` | 挂载 `/v1`；须与 `NEW_API_ENABLED` 同开才有意义 |
...
| failure 队列与 relay outbox 分离 | 入账失败与 Token 同步失败的重试语义不同 |
