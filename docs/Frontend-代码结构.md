...
└─ AdminLayout
   ...
   ├─ QueryProvider
   ├─ USE_MOCKS ?
   │    └─ LazyDemoShell → DemoSessionProvider + DemoGuide + DemoRoleNavigationBridge
   └─ AuthSessionProvider + AuthUnauthorizedBridge + SessionNavigationBridge（生产 cookie/JWT）
   └─ WorkflowProvider
...
常量与环境：`config/routes.ts`、`config/app.ts`、`lib/date.ts`、`features/workflow/constants.ts`、`features/demo/guide/constants.ts`。Fixture 时间锚点仅放 `mocks/lib/demo-clock.ts`。

**Demo 墙钟说明：** 业务 UI（`lib/date.ts`）使用真实本地日期；fixture 时间戳锚定在 `mocks/lib/demo-clock.ts`（如 `2026-06-19`）。审计页默认筛选为「全部时间」，可看到 fixture 数据；若用户选择「近 7 天」，范围按墙钟计算，当系统日期与 fixture 锚点相差较远时可能筛不到记录，需改选「全部时间」或扩大范围。MSW 层不做 `DEMO_TODAY` 补偿。
...
Demo session 加载失败时通过 `sessionError` + `SessionGate` 展示 `ErrorState`，不静默清空为无权限空会话。
...
| 业务 hook 引用 `mocks/lib/demo-clock` | `lib/date.ts` 用真实日期；fixture 锚点仅 `mocks/lib/demo-clock.ts`                 |
| `lib/` 含 React 组件                  | 迁至 `components/{domain}/`（如 keys badge → `components/keys/status-badges.tsx`） |
| Mock handler 内联假业务数据           | 查 `fixtures/` + 请求上下文（如 `X-Demo-Member-Id`）                               |
