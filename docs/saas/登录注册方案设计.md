...
sequenceDiagram
    ...
    B->>B: 创建 User (phone, password_hash=空)
    B->>B: CreateCompany(type='trial') → company + inviteCode
    B->>B: AcceptInvite(inviteCode, name=phone, password=random)
    Note over B: 复用现有 CreateCompany + AcceptInvite 逻辑
    B->>B: 灌入试用资金 (TRIAL_CREDIT_AMOUNT)
    B->>B: 签发 Session Cookie
    B-->>F: { memberId, companyId, needsOnboarding: true }
```

**实现策略**：复用 `company.CreateCompany` + `company.AcceptInvite`，减少重复代码。CreateCompany 已处理：公司创建 + 钱包开通 + 角色初始化 + org tree + 邀请码生成。
...
sequenceDiagram
    ...
    B->>B: CreateCompany(type='selfhosted', superAdminEmail=email)
    B->>B: AcceptInvite(inviteCode, name, password)
    B->>B: 可选灌入示例数据
    B->>B: 签发 Session Cookie
    B-->>F: { memberId, companyId }
```

同样复用 `CreateCompany + AcceptInvite`。
...
```go
// OrgRepository 新增（跨 company 的全局查询，不走 company context）
MembersByUserID(ctx context.Context, userID uuid.UUID) ([]types.Member, error)
```

现有 `UserRepo.GetByPhone` / `UserRepo.GetByEmail` 已满足需求。
...
2. `OrgRepo.MembersByUserID()` — 跨 company 全局查询（不走 company context）
3. `POST /auth/sms/send` + `POST /auth/sms/verify` + `POST /auth/sms/select`
4. `POST /auth/setup` — 复用 CreateCompany + AcceptInvite
5. `POST /auth/register` — 复用 CreateCompany + AcceptInvite + 灌试用资金
6. 前端 `/login` 改造 + `/setup` + `/register` 页面
7. 前端 `companyId: number` → `string` 全量修正

### Phase 2 — 邮箱登录改造 + Onboarding

8. `POST /auth/login/v2`（无需 companyId）→ 复用 MembersByUserID 逻辑
9. `companies.onboarding_status` + `GET /session` 携带
10. Onboarding Workflow（飞书 / CSV / 手动导入）
11. `/invite/accept` 前端页面
