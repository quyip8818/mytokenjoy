...
**相关：** [Backend.md](./Backend.md) · [Backend-架构.md](./Backend-架构.md) · [Backend-预算.md](./Backend-预算.md) · [Backend-存储实体优化.md](./Backend-存储实体优化.md)
...
| 种子     | `apps/backend/seed/`（设计见 [Backend-seed.md](./Backend-seed.md)）；demo 运行态 `runtime.ApplyUsageBuckets` |
...
| 消耗字段 | `budget_snapshots`（`axis_kind=org_node`） | `budget_snapshots`（`axis_kind=budget_group`） |
...
- `relay_mappings`：仅 Relay 同步状态（`platform_key_id` ↔ NewAPI `token_id`）；成员/部门/组归因经 `platform_keys` JOIN。
- `platform_keys.key_hash`：Gateway Bearer 鉴权；`full_key` 不落库，仅 create/rotate 响应一次。
...
flowchart LR
  ...
  PROJ --> SNAP[(budget_snapshots)]
  PROJ --> UB[(usage_buckets)]
  DASH[看板 GET] --> UB
  DASH -->|minute| UL
  GW[Gateway 预检] --> SNAP
  GW --> CFG[org_nodes / budget_groups / platform_keys 配置]
...
| `usage_buckets`    | hour/day 聚合               | Ingest 投影           | 看板 cost/consumed 趋势       |
| `budget_snapshots` | 各轴 `consumed` 缓存        | Ingest 投影           | Gateway 预检、预算树、Overrun |
...
| 列组 | 字段                                                               | 说明                                        |
| ---- | ------------------------------------------------------------------ | ------------------------------------------- |
| 树   | `id`, `parent_id`, `path` (ltree), `name`, `external_id`, `source` | 邻接表 + ltree 祖先链；读出组装嵌套树       |
| 预算 | `budget`, `reserved_pool`, `period`                                | `consumed` 读 `budget_snapshots`            |
| 路由 | `default_model_id`, `fallback_model_id`, `routing_inherited`       | 白名单在 `model_allowlist`（`model_id` FK） |

**Repository 分工：**

| 操作                          | 入口                        |
| ----------------------------- | --------------------------- |
| 树 CRUD / ltree `path` 维护   | `Org().Nodes()`             |
| 改 `budget` / `reserved_pool` | `budget.Service.UpdateNode` |
| 改路由列                      | `models.Service`            |
| 白名单                        | `Models().Allowlist()`      |
| 读 `consumed`                 | `BudgetSnapshots()`         |
...
### 7.4 `async_jobs` — Relay / Rebalance / Overrun 统一队列

| 列           | 说明                                    |
| ------------ | --------------------------------------- |
| `channel`    | `relay` \| `rebalance` \| `overrun`     |
| `dedupe_key` | rebalance 轴去重（`axis_kind:axis_id`） |
| `payload`    | JSONB；relay 含 `company_id`            |

Worker `runner.go` 按 `channel` claim；取代原 `outbox` / `rebalance_queue` / `overrun_queue`。
...
| 预算   | `org_nodes`（预算列）, `budget_groups`, `budget_snapshots`, `budget_group_members`, `budget_group_departments`, `overrun_policy`, `alert_rules`, `alert_rule_notify_roles` |
...
| 运行面 | `usage_buckets`, `budget_snapshots`, `async_jobs`, `scheduler_locks`, `ingest_failures`, `reconcile_cursors`                                                               |
...
| 表                  | 职责                                                                                   |
| ------------------- | -------------------------------------------------------------------------------------- |
| `budget_snapshots`  | `(company_id, axis_kind, axis_id, period_key)` → `consumed`                            |
| `relay_mappings`    | 窄表：仅 Relay 同步状态；归因 JOIN `platform_keys` / `members`                         |
| `ingest_failures`   | 入账失败重试队列                                                                       |
| `reconcile_cursors` | 全局 logs 库 reconcile 水位                                                            |
| `async_jobs`        | `channel` = relay / rebalance / overrun                                                |
| `scheduler_locks`   | Worker 单实例锁                                                                        |
| `usage_buckets`     | `(bucket_start, department_id, member_id?, model)` 聚合；`member_id` 可 NULL；按月分区 |
| `usage_ledger`      | 消耗事件 SSOT；`PARTITION BY RANGE (occurred_at)`                                      |
| `operation_logs`    | 操作审计；`PARTITION BY RANGE (created_at)`                                            |

分区子表由 [`schema_partitions.go`](../apps/backend/internal/store/postgres/schema_partitions.go) 在 apply 时批量创建（2024-01 .. 2028-12）；范围外 INSERT 需扩分区或重建 volume。
