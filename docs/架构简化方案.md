...
| 包 / 路径 | 职责 | 简化方案中的位置 |
| --- | --- | --- |
| `domain/gateway/` | `/v1` 路由、反代、预检 | **Phase 1 已落地** |
...
| `domain/company/` | 租户上下文、`WalletService`（**读 NewAPI 配额**） | **Phase 1 完成**：Gateway 已退出热路径；Phase 3 冷路径保留 |
...
| `pkg/budget/` | `RemainForMapping`、开账月 | **Phase 1 完成**：热路径迁入 `Evaluate`（`ComputeRemainBudget`） |

> **Redis**：代码中**尚未存在**；Phase 5 新增 `PrecheckCache` 抽象与 `REDIS_URL` 配置。

### 0.2 `/v1` 热路径（Phase 1 已落地）

**当前实现**（`gateway_service.go` + `precheck.go` + `gateway_precheck_repo.go`）：
...
| 指标 | Phase 1 前 | 当前（Phase 1 后） | 后续（Phase 2+） |
| --- | --- | --- | --- |
| Gateway 层 store 调用 | **≥ 3** | **1**（`LoadPrecheckContext`） | 不变；Phase 5 可选 Redis |
| 外部 HTTP 预检 | **有**（NewAPI quota） | **0** | 不变 |
| 钱包余额读法 | `balance_point`（O(1)） | 同左 | rename → `wallet_remain` |
| 预检对象 | `PrecheckInput` | `PrecheckContext` 四域 | Phase 4 扩 `PolicyState` |
...
| lot + 投影写 | 分散：`usage/lot_allocate.go` + `billing_repo` | 收口 **`WalletProjectionService`**（Phase 2） |
| 预检服务 | ~~`PrecheckService`（7 步检查）~~ | **`LoadPrecheckContext` + `Evaluate()`**（Phase 1 已落地） |
| `company.WalletService` | ~~Gateway 热路径注入~~ | **仅** PlatformSync / 对账冷路径（Phase 1 已完成） |
| 异步同步 | River kinds：`wallet_sync`、`rebalance`、`newapi_sync` | 远期可合并 **`PlatformSync(company_id)`** |
| 预算 snapshot 轴 | 四轴：`org_node` / `budget_group` / `platform_key` / `member` | **保留四轴**（PRD：member/group 挡单） |
| `org_nodes` | 结构 + `budget` / `reserved_pool` + 路由字段同表 | 拆 `org_node_budget`；路由 → `model_allowlist` |
| `usage_buckets` | Ingest 仍写（`usage/projection.go`） | **停写**；Dashboard 读 ledger（**可慢**） |
...
| Phase | 主要改动文件（示意） |
| --- | --- |
| **1 Gateway** | `gateway_service.go`、`precheck.go`、`precheck_context.go`、`evaluate.go`；`store/postgres/gateway_precheck_repo.go`（**已落地**） |
...
### Phase 1 — Gateway（`/v1`）— **状态：已落地（2026-07）**

**目的：** 预检 = 1 次 Repository 调用 + `Evaluate()` + 反代。

**验收（均已满足）：**

- Gateway 层 ≤1 store 调用；预检 0 NewAPI HTTP
- 读 `balance_point` 投影列；`tests/domain/gateway/` 与 handler 集成测通过
- 已删除 `checkNewAPIWalletCap`、`checkWalletSyncLag`、`checkNewAPIKeyRemainQuota`

**Phase 1 前 vs 后：**

| 项 | Phase 1 前 | Phase 1 后（当前） |
| --- | --- | --- |
| 入口 | mapping + company + `PrecheckService.Run` | `LoadPrecheckContext(key_hash)` |
| 预检 | 7 步含 NewAPI HTTP / sync lag 503 | `Evaluate(PrecheckContext)` 纯内存 |
| NewAPI 字段 | `NewAPIKeyRemainQuota` 参与预检 | 热路径不读；quota 由 PlatformSync 维护（Phase 3） |
| 预算 remain | `RemainForMapping` 多次查询 | 预检 SQL 一次带出 consumed + limit |
...
**目的：** 拆 org 胖表、钱包写收口、停写 buckets 解耦看板。  
**硬约束 ①：`/v1` 效率不得回退。** **硬约束 ②：member / budget_group cap 挡单不变（PRD）。**  
Dashboard **允许变慢**。  
**专篇**：[架构简化-Phase2详解.md](./架构简化-Phase2详解.md)

| 项 | 动作 |
| --- | --- |
| `org_nodes` | 拆 `org_node_budget`；路由 → `model_allowlist` |
| `wallet_remain` | rename；`WalletProjectionService` 写收口 |
| `budget_snapshots` | **保留四轴** Ingest 写 + `/v1` 读（member/group 挡单） |
| `LoadPrecheckContext` | JOIN `org_node_budget`；**保留四轴 snapshot JOIN** |
| `Evaluate` | **四轴 min 不变** |
| `usage_buckets` | Ingest **停写**；看板 ledger 聚合 |

**不做：** 四轴→两轴；`/v1` 扫 ledger；删 `wallet_remain`；削弱 member/group 挡单。

**Ingest 写路径：**

```mermaid
flowchart LR
  L[usage_ledger] --> S[四轴 snapshot + org rollup]
  L --> WS[WalletProjectionService]
```

**验收：** `/v1` P99 ≤ 基线；**member/group 挡单测绿**；Dashboard 功能正确。
...
| 队列 | River `river_job`；outbox kind `newapi_sync` | 远期统一 `platform_sync` debounce |
...
> **钱包：写路径 WalletService 写 lot + 投影；Ingest 写 ledger + 四轴 snapshot（PRD 挡单不变）。**  
