...
| 认证方式 | 邮箱 + 密码 | 手机号验证码 |
| Company type | `selfhosted` | `standard` |
...
> Demo 模式的完整技术方案见 [Demo 流程实施方案](./Demo流程实施方案.md)，以下仅做概要。

### 8.1 核心设计

Demo 为匿名体验，无需手机号注册。通过 JWT Cookie 绑定 Demo 租户，30 天无活动清理。
...
| --- | --- | --- |
| 入口 | "免费试用" 按钮 | "登录/注册" 按钮 |
| 认证 | 匿名（Cookie 绑定） | 手机号 + 验证码 |
| Company type | `demo` | `standard` |
| 数据 | 预灌 seed 或 CSV 导入 | 空白 + Onboarding |
| 生命周期 | 30 天无活动清理 | 永久 |
...
-- Demo 租户通过 companies.type = 'demo' 区分，无需额外映射表。
-- 详见 Company租户模型设计.md。
...
| Demo 被滥用 | IP 限流 + `DEMO_MAX_TENANTS` 上限；advisory lock 防并发超发 |
| Demo Cookie 安全 | JWT 签名校验；company_id 在 JWT claims 中 |
...
6. **Demo 流程**
   - `POST /auth/demo/create`（见 Demo 流程实施方案）
   - 清理 River job（基于成员最近登录时间）
...
| `companies.type` 列 | 已实现，区分 selfhosted/testing/demo/standard/trial |
| 前端 DEV 自动填充 | 保留 |
