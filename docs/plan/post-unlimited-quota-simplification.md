...
> 日期：2026-07-20  
> 状态：**大部分已完成** — model_limits 同步已移除（见 `docs/plan/remove-newapi-modellimits-sync.md`），create/update.go 已简化。  
...
| **bootstrap.go ensureWalletCompanyQuota** | TopUp to wallet_quota_remain | ✅ 已改为正确逻辑 |
...
| B: bootstrap 一次性补齐 | 启动时 TopUp 到 wallet_quota_remain | 简单 | 两次启动之间的充值不会反映到 NewAPI |
...
- `DepartmentID` 直接从 `existing.DepartmentID` 读取
...
| 应用启动 | `TopUp(walletCompanyID, max(0, walletQuotaRemain - currentQuota))` | `provision/bootstrap.go` |
...
| bootstrap TopUp to wallet_quota_remain | 启动前的消费可能让 NewAPI quota < tokenjoy wallet_quota_remain | 不可能 — NewAPI 消费和 tokenjoy ingest 扣同样的值 |
| 充值后 TopUp 失败 | NewAPI user quota < wallet_quota_remain | 可接受 — 下次充值/启动补齐；Gateway precheck 仍正常 |
| 充值后 TopUp 重复执行 | NewAPI user quota > wallet_quota_remain | 安全 — Gateway precheck 在 NewAPI 之前拦截；多余 quota 不造成安全风险 |
...
**读 wallet_quota_remain 路径**：`d.Store.Company().GetByID(ctx, companyID)` → `co.WalletQuotaRemain`

| 文件 | 改动 |
|------|------|
| `provision/bootstrap.go` | 新建 user 时 `Quota: co.WalletQuotaRemain`；已有 user 时 `TopUp(walletCompanyID, max(0, co.WalletQuotaRemain - currentQuota))` |
...
| 文件 | 改动 |
|------|------|
| 现有 bootstrap test | 断言调用 TopUp with wallet_quota_remain delta |
...
- Gateway precheck 逻辑 — 不变（wallet_quota_remain + combined_key_remain 两层检查）
...
┌─────────────────────────────────┐
│ NewAPI                          │  ← 物理止损层
│  • user quota > 0  ← 由 TopUp 保证 ≥ wallet_quota_remain
│  • token unlimited ← hardcode true
└─────────────────────────────────┘
```

两层防线的不变量：`NewAPI user quota ≥ tokenjoy wallet_quota_remain`
