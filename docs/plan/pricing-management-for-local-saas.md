...
| 启停 | ✅ | ✅ |
| 配置路由/白名单 | ✅ | ✅ |
| Channel 管理（上游渠道） | — | ✅（客户自有 API Key 接入） |

**Channel 管理说明**：Local 客户可以添加自己的上游渠道（ProviderKey），比如客户自有的 OpenAI / Anthropic API Key。通过 TJ 自研 UI 完成 CRUD，后端同步到本地 NewAPI 的 channel 配置。SaaS 模式下此功能被 `SupportSaas` 标志禁用。
...
| **Channel 管理** | ✅（上游供应商接入） | ✅（客户自有上游渠道） | ❌ 不暴露 |
| **自定义模型** | — | ✅ 客户自己的模型 | ❌ |
...
| Local 用户可能做的 | 平台账单影响 |
|-------------------|-------------|
| 改本地 NewAPI 的 model_ratio | ❌ 不影响（平台网关按自己的 ratio 计费） |
| 停止同步平台价格 | ❌ 不影响（本地展示偏差，不影响平台计费） |
| 修改 TJ 后端代码 | ❌ 不影响 |
| 添加自有 channel（客户自有 API Key） | 走自有 channel 的流量不经平台 → 平台不计费 → 正常行为 |
| 用自有 channel 替换平台 channel | 该模型流量不走平台 → 平台不出账 → 合同约束问题 |
