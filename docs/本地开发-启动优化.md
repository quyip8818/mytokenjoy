...
| **`pnpm start:local:empty`** | L0（prod mode） | `ensure-infra` **轻** → backend（`BOOTSTRAP_MODE=prod`）+ frontend + mock | 空环境，仅 admin 可登录 |
...
| 5 | Channel | `setup-dev-mock-channel.sh` | `local-test-model` + `pass_through_body_enabled`（best-effort） |
...
| 入口 | 分发 | 组合脚本 |
| --- | --- | --- |
| `scripts/dev.sh` | `local` \| `local-empty` \| `saas` \| `lite` \| `reset` \| `infra` \| `test` | `scripts/dev/*.sh` |
| `package.json` | `docker:reset` / `reset` → `dev.sh reset` | 同左 |
| Infra 重 / 轻 | — | `apps/newapi/scripts/start-infra.sh` / `ensure-infra.sh` |

```text
scripts/dev.sh → scripts/dev/start-local.sh       → ensure-infra + concurrently
              → scripts/dev/start-local-empty.sh → ensure-infra + concurrently (BOOTSTRAP_MODE=prod)
...
River producer 心跳日志？
  └─ 已过滤（`Producer job counts`；stuck>0 仍会打）；River 照常开
```
...
pnpm start:local:empty     # 空环境（prod mode，仅 admin，无 demo 数据）
