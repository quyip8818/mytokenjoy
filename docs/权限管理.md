...
> **交付策略**：**破坏性替换**，不保留向后兼容，不提供渐进 migration。旧实现（裸 member ID、`APP_PROFILE` 鉴权分叉、前端双份 PDP）在切换日整体删除。  
> **文档范围**：下文描述 **当前已实现** 的目标态行为与契约；§2 保留切换前后对照供回顾。实现索引见 §14.4。
...
| P7  | 管理台 Session 与 Relay `sk-xxx` **永久分离**（不同 AuthN 体系）                                                                  |
...
**契约层**（非运行时 PDP）：`internal/infra/permission/manifest.json` → 生成 `keys.go` / `permission-keys.ts`。
...
**路径**：`apps/backend/internal/infra/permission/manifest.json`
...
| Relay Gateway | `Bearer sk-xxx` | `relay_mappings` + 预检 |
...
| 删除                       | 原因                      |
| -------------------------- | ------------------------- |
| `APP_PROFILE` 用于鉴权分叉 | 统一 Session + capability |
| 任何「demo 免鉴权」开关    | 破坏性统一                |

> `APP_PROFILE` 若仍用于非鉴权用途（如 `SIMULATE_DELAY`），可保留名称但 **不得** 影响 AuthN/Authz。
...
cd apps/backend && go generate ./... && go test ./...
...
| 模块               | 路径                                                                                                                                                      |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Manifest           | `internal/infra/permission/manifest.json`                                                                                                                 |
...
| PAP                | `internal/domain/org/role.go`、`member.go`、`authz_bump.go`                                                                                               |
...
| 组合根             | `internal/app/wire_identity.go`、`wire_domain_services.go`、`wire_relay.go`、`registry.go`                                                                |
