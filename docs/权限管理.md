...
```json
{
  "sub": "00000000-0000-7000-8000-000000000e01",
...
**DI**：`app/compose_http.go` 装配；`ServiceRegistry` 嵌入 `httpdeps.Deps`（见 §14.4）。
...
```json
{
  ...
  "member": {
    ...
    "name": "管理员",
    "roles": ["组织管理员"],
    "departmentId": "00000000-0000-7000-8000-000000000d01",
    "status": "active"
  },
...
| 5.6 | Wallet 路由 + Billing API + Gate | ⚠️ `/wallet` + `billingApi.getWallet` / `recharge` / `confirmRecharge` 已接入；`recharge-records` 半真数据见 [plan.md](./plan.md) §2 |
...
| `/api/keys/provider`、`/platform`、`/platform/budget-summary` | `keys:read`                         |
...
| 组合根             | `internal/app/assemble.go`、`compose_infra.go`、`compose_domain_wire.go`、`compose_http.go`、`compose_worker.go`、`registry.go` |
