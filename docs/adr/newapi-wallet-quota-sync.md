...
| 层级 | 检查时机 | 我们的策略 |
|------|---------|-----------|
| User quota | 每次 API 调用 | 需要保持 ≥ tokenjoy wallet_quota_remain |
| Token quota | 每次 API 调用 | `unlimited_quota=true`，不检查 |

tokenjoy Gateway precheck 是业务限额控制面（combined_key_remain + wallet_quota_remain），NewAPI user quota 是**物理止损层**——防止绕过 Gateway 直连 NewAPI 时的无限消费。
...
```
tokenjoy wallet_quota_remain = SSOT（单一事实源）
NewAPI user quota = 物理止损镜像（≥ wallet_quota_remain 即可）
```
...
if currentQuota < co.WalletQuotaRemain {
    client.TopUp(ctx, { CompanyID: walletCompanyID, Quota: co.WalletQuotaRemain - currentQuota })
}
```

语义：**保证 NewAPI user quota ≥ tokenjoy wallet_quota_remain**。不用 MaxInt32 硬编码。
...
| `newapisync/provision/bootstrap.go` | 已有 user 时 TopUp 到 `wallet_quota_remain`（替换 MaxInt32 hacky 逻辑） |
...
后果：NewAPI user quota > tokenjoy wallet_quota_remain。**这没问题**——因为 Gateway precheck（读 PG wallet_quota_remain）在 NewAPI 之前拦截，真正的限额由 tokenjoy 控制。多余的 NewAPI quota 只是"宽松了一点"，不造成安全风险。
...
NewAPI 消费时**自动扣减** user quota（这是 NewAPI 内部行为）。tokenjoy ingest 独立地从 lot 扣减 wallet_quota_remain。两者各算各的，不需要交叉同步。

**关键不变量**：`NewAPI user quota ≥ tokenjoy wallet_quota_remain` 在正常运行时始终成立（因为 NewAPI 消费扣减的 quota 和 tokenjoy ingest 扣减的 wallet_quota_remain 源自同一个 consume_log.quota，数值相等）。

### 4. Overdraft

Overdraft lot 扩展时（`ExpandOverdraftLot`），tokenjoy wallet_quota_remain 保持 0（overdraft 借贷不增加真实余额），所以 **不需要 TopUp**。NewAPI 侧的 user quota 此时可能已经是 0 或负值——这导致 NewAPI 拒绝新请求，和 tokenjoy Gateway precheck 的 "wallet_quota_remain ≤ 0 → 拒绝" 行为一致。不需要额外处理。

### 5. Trial 充值（MockLot）

`SeedTrialCredit` 创建 mock lot 时也走 `CreditFromLot`，加上 TopUp 后 NewAPI user quota 也会增加。Trial → Standard 升级时 `ExpireMockLots` 减少 wallet_quota_remain，但不减少 NewAPI user quota。这没问题——多余 quota 在正常消费中自然扣减；且下次 bootstrap 会修正。
...
| Bootstrap 启动 user quota=0, wallet_quota_remain=50M | TopUp delta=50,000,000 |
| Bootstrap 启动 user quota=100M, wallet_quota_remain=50M | 不 TopUp（quota 已 ≥ remain） |
| Overdraft 扩展 | 不 TopUp（wallet_quota_remain 不变） |
