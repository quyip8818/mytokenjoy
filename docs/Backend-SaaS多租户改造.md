...
| ---------- | --------------------- | ---------- | ------------------------ | --------------------- | ------------------------ |
| **私有化** | `MULTI_COMPANY=false` | 大企业自建 | 企业超管 `provider_keys` | `dept-{departmentId}` | 一企业服务账户           |
| **SaaS**   | `MULTI_COMPANY=true`  | 平台多客户 | 仅平台运营               | `platform_shared`     | **每企业**一企业服务账户 |
...
4. **策略 + DI** 切换模式，业务层不散落 `if cfg.MultiCompany`。
...
私有化（`MULTI_COMPANY=false`）：行为不变；钱包与 budget 两条轴同样成立，仅无多企业隔离。
...
| 私有化 `MULTI_COMPANY=false` | 固定 `DEFAULT_COMPANY_ID`（默认 `1`）                                      |
...
`/api/platform/*`：仅 `MULTI_COMPANY=true`；走 **PlatformAuth** middleware，与企业 Session 隔离。
...
- 单租户默认 `testutil.Ctx()` 测 SaaS 行为却不设 `MultiCompany=true`
...
| MultiCompany 路由契约                    | `tests/handler/contract_test.go`                          | P2     |
...
| Helper                      | 用途                                                   |
| --------------------------- | ------------------------------------------------------ |
| `ApplySaaSConfig(cfg)`      | 开启 `MultiCompany` + platform bootstrap               |
