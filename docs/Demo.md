...
> 角色视角：产品设计师  
> 依据：[TokenJoy-PRD.md](./TokenJoy-PRD.md)  
> 范围：前端 Demo（P1～P4），MSW Mock；不含真实后端与 US-12 API 网关  
> 核心模式：**列表/看板为主画布 + 右侧 Workflow Panel（可多层嵌套）**  
> 更新日期：2026-06-25  
> 实施进度见 [§15](#15-实施进度与验收清单)；PRD 差距见 [§18](#18-prd-vs-demo-差距分析)
...
_面板宽度：Layer 1 **75vw** · Layer 2 **62vw** · Layer 3 **50vw**。实施进度见 [§15](#15-实施进度与验收清单)；PRD 差距见 [§18](#18-prd-vs-demo-差距分析)。_
...
#### 18.2.2 简版（有 UI，规则为 Mock 或未全量验收）
