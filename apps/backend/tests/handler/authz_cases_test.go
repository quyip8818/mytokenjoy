...
func authzWriteCases() []authzCase {
	...
	return []authzCase{
		...
		{name: "budget update forbidden", method: http.MethodPut, path: "/api/budget/departments/" + seed.IDDept3, body: `{"budget":1000}`, cookie: testutil.SessionCookie(seed.IDMemberPure), wantStatus: http.StatusForbidden},
		{name: "budget update allowed", method: http.MethodPut, path: "/api/budget/departments/" + seed.IDDept3, body: budgetUpdateBody, cookie: sessionCookie, wantStatus: http.StatusOK},
