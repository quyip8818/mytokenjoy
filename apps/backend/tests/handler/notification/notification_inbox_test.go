...
func TestNotificationInbox_MarkAllRead(t *testing.T) {
	t.Parallel()
	router := testhttp.NewRouter(t)
	cookie := testhttp.AdminCookie(t)
	...
	seedNotification(t, router, cookie, types.NotificationLogEntry{
		...
		Title:     "Key Expiring",
		Body:      "prod-key expires in 7 days",
	})
	seedNotification(t, router, cookie, types.NotificationLogEntry{
		UserID:    contract.IDMemberAdmin,
		EventType: "budget_alert_reached",
	...
	router.ServeHTTP(rec, req)
	if rec.Code != http.StatusNoContent && rec.Code != http.StatusOK {
		t.Fatalf("mark all read: expected 2xx, got %d: %s", rec.Code, rec.Body.String())
	}
	...
	if countResp.Count != 0 {
		t.Fatalf("expected 0 unread after mark-all-read, got %d", countResp.Count)
	}
}

func TestNotificationInbox_ArchiveAll(t *testing.T) {
	t.Parallel()
	router := testhttp.NewRouter(t)
	cookie := testhttp.AdminCookie(t)
	...
	// Archive all
	req := httptest.NewRequest(http.MethodPost, "/api/notifications/archive-all", nil)
	req.Header.Set("Cookie", cookie)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
	if rec.Code != http.StatusNoContent && rec.Code != http.StatusOK {
		t.Fatalf("archive all: expected 2xx, got %d: %s", rec.Code, rec.Body.String())
	}
	...
	if len(resp.Items) != 0 {
		t.Fatalf("expected empty inbox after archive-all, got %d items", len(resp.Items))
	}
}
