...
func TestCoreHandler(t *testing.T) {
	...
	t.Run("SessionUnauthorized", func(t *testing.T) {
		t.Parallel()
		cases := []struct {
			name   string
			cookie string
	...
	t.Run("SessionDemoSuccess", func(t *testing.T) {
		t.Parallel()
		req := httptest.NewRequest(http.MethodGet, "/api/session", nil)
	...
	t.Run("UnknownAPIRouteReturnsJSON", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("KeysPlatformCreateMissingMemberID", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("BudgetNodeUpdateOversell", func(t *testing.T) {
		t.Parallel()
