...
func testKeyBudgetExhaustion(t *testing.T) {
	...
	budgetfix.SetPlatformKeyBudgetState(t, env.Store, contract.IDPlatformKey1, 100, 95)
...
func testConcurrentIngestRace(t *testing.T) {
	...
	budgetfix.SetPlatformKeyBudgetState(t, env.Store, contract.IDPlatformKey1, 100, 90)
...
// testWalletInsufficientGatewayReject verifies that when wallet_remain_quota drops
...
func testWalletInsufficientGatewayReject(t *testing.T) {
	...
	// Set wallet_remain_quota to 0
	ctx := testutil.Ctx()
	if err := env.Store.Company().SetWalletRemainQuota(ctx, contract.DefaultCompanyID, 0, nil); err != nil {
...
func TestBenchmarkGatewayPrecheck(t *testing.T) {
	...
	// Warm up
	_, _ = env.Precheck.Run(testutil.Ctx(), keyHash, "test-model", gateway.PrecheckOpts{SkipModelAllowlist: true})
