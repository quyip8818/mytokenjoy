...
func TestMergeTokenPutPreservesAndHealsExpired(t *testing.T) {
	...
	remain := int64(42)
	got := newapi.MergeTokenPut(cur, adminport.UpdateTokenInput{ID: 9, RemainQuota: &remain})
	...
	healed := newapi.MergeTokenPut(broken, adminport.UpdateTokenInput{ID: 9, RemainQuota: &remain})
	...
	forced := newapi.MergeTokenPut(cur, adminport.UpdateTokenInput{ID: 9, ExpiredTime: &explicit})
...
func TestMergeChannelPutPreservesUnspecified(t *testing.T) {
	...
	got := newapi.MergeChannelPut(cur, adminport.UpsertChannelInput{ID: 12, Status: 2, Key: "sk-new"})
