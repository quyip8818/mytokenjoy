...
func TestGetQuotaRemaining(t *testing.T) {
	...
	remaining := memberquota.GetQuotaRemaining(pools, keys, seed.IDMember1)
	if remaining != 3000 {
...
func TestBuildQuotaSummary(t *testing.T) {
	...
	pools := snapshot.MemberQuotaPools
	keys := snapshot.PlatformKeys
	reserved := budgetlookup.GetReservedPoolForMember(snapshot.BudgetTree, snapshot.Members, seed.IDMember1)

	summary := memberquota.BuildQuotaSummary(pools, keys, seed.IDMember1, reserved)
