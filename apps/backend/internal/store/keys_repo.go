...
type KeysRepository interface {
	...
	// PlatformKeys returns ALL keys including deleted ones.
	// DO NOT add status filter here — SetPlatformKeys prune depends on the full set.
	// See: docs/adr/platform-key-soft-delete.md
	...
	SoftDeletePlatformKey(ctx context.Context, keyID uuid.UUID) error
