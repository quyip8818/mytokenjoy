...
const memberSelect = `
	SELECT m.id, m.user_id, m.alias, m.avatar, COALESCE(m.department_id, '00000000-0000-0000-0000-000000000000'::uuid), COALESCE(o.name, ''), m.status, m.source, m.registration_channel, m.external_id, COALESCE(m.employee_id, ''), COALESCE(m.job_title, ''), COALESCE(m.hire_date, ''), m.override_fields, m.personal_budget,
		COALESCE(u.name, ''), COALESCE(u.phone, ''), COALESCE(u.email, '')
	FROM members m
...
const memberListSelect = `
	SELECT m.id, m.user_id, m.alias, m.avatar, COALESCE(m.department_id, '00000000-0000-0000-0000-000000000000'::uuid), COALESCE(o.name, ''),
		m.status, m.source, m.registration_channel, m.external_id, COALESCE(m.employee_id, ''), COALESCE(m.job_title, ''), COALESCE(m.hire_date, ''), m.override_fields, m.personal_budget,
		COALESCE(u.name, ''), COALESCE(u.phone, ''), COALESCE(u.email, ''),
...
func (r *pgOrgRepo) Members(ctx context.Context) ([]types.Member, error) {
	...
	rows, err := r.db.Query(ctx, memberListSelect+`
		...
		GROUP BY m.id, m.user_id, m.alias, m.avatar, m.department_id, o.name,
			m.status, m.source, m.registration_channel, m.external_id, m.employee_id, m.job_title, m.hire_date, m.override_fields, m.personal_budget,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&item.DepartmentID, &item.DepartmentName, &item.Status, &item.Source, &item.RegistrationChannel, &item.ExternalID,
			&item.EmployeeID, &item.JobTitle, &item.HireDate, &item.OverrideFields, &item.PersonalBudget,
			&item.Name, &item.Phone, &item.Email, &roles,
		); err != nil {
			return nil, err
		}
...
func (r *pgOrgRepo) MemberByEmail(ctx context.Context, companyID uuid.UUID, email string) (*types.Member, string, error) {
	row := r.db.QueryRow(ctx, `
		SELECT m.id, m.user_id, m.alias, m.avatar, COALESCE(m.department_id, '00000000-0000-0000-0000-000000000000'::uuid), COALESCE(o.name, ''), m.status, m.source, m.registration_channel, m.external_id, COALESCE(m.employee_id, ''), COALESCE(m.job_title, ''), m.override_fields, m.personal_budget, u.password_hash
		FROM members m
		JOIN users u ON u.id = m.user_id
		LEFT JOIN org_nodes o ON o.company_id = m.company_id AND o.id = m.department_id
...
func (r *pgOrgRepo) GetMemberAuthz(ctx context.Context, companyID uuid.UUID, memberID uuid.UUID) (*store.MemberAuthz, error) {
	row := r.db.QueryRow(ctx, `
		SELECT m.id, m.user_id, m.alias, m.avatar, COALESCE(m.department_id, '00000000-0000-0000-0000-000000000000'::uuid), COALESCE(o.name, ''), m.status, m.source, m.registration_channel, m.external_id, COALESCE(m.employee_id, ''), COALESCE(m.job_title, ''), COALESCE(m.hire_date, ''), m.override_fields, m.personal_budget,
		       u.name, COALESCE(u.phone, ''), COALESCE(u.email, ''), c.authz_revision
		FROM members m
		JOIN users u ON u.id = m.user_id
	...
	if err := row.Scan(
		...
		&item.DepartmentID, &item.DepartmentName, &item.Status, &item.Source, &item.RegistrationChannel, &item.ExternalID,
		&item.EmployeeID, &item.JobTitle, &item.HireDate, &item.OverrideFields, &item.PersonalBudget,
		&item.Name, &item.Phone, &item.Email, &revision,
	); err != nil {
		if err == pgx.ErrNoRows {
			return nil, nil
		}
	...
	for i, member := range cloned {
		...
		if _, err := r.db.Exec(ctx, `
			INSERT INTO members (
				...
				status, source, registration_channel, external_id, employee_id, job_title, hire_date, override_fields, personal_budget, updated_at
		...
		`, member.ID, companyID, member.UserID, member.Alias, member.Avatar,
			nilUUID(member.DepartmentID), member.Status, member.Source, member.RegistrationChannel, member.ExternalID,
			nilText(member.EmployeeID), nilText(member.JobTitle), nilText(member.HireDate), member.OverrideFields, member.PersonalBudget); err != nil {
