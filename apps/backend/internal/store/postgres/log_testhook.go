...
func ListNotificationLogs(ctx context.Context, pool *pgxpool.Pool, companyID uuid.UUID) ([]types.NotificationLogEntry, error) {
	rows, err := pool.Query(ctx, `
		SELECT id, channel, event_type, COALESCE(user_id, '00000000-0000-0000-0000-000000000000'::uuid), payload, send_ok, COALESCE(error, '')
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.ID, &e.Channel, &e.EventType, &e.UserID, &e.Payload, &e.SendOK, &e.Error); err != nil {
			return nil, err
		}
		out = append(out, e)
	}
