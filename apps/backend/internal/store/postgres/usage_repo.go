...
const (
	usageBucketColumns = `bucket_start, department_id, member_id, model,
			quota_consumed, cost, call_count, input_tokens, output_tokens`
)
...
func (r *usageRepo) UpsertBucket(ctx context.Context, row types.UsageBucketRow) error {
	...
	_, err := r.db.Exec(ctx, `
		INSERT INTO usage_buckets (
			...
			quota_consumed, cost, call_count, input_tokens, output_tokens, updated_at
	...
	`, companyID, row.BucketStart.UTC(), row.DepartmentID, memberID, row.Model,
		row.QuotaConsumed, row.Cost, row.CallCount, row.InputTokens, row.OutputTokens)
	return err
}
