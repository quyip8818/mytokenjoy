...
func (r *gatewaySoftSummaryRepo) DecrementBatch(ctx context.Context, decrements map[string]float64) ([]store.GatewaySoftSummary, error) {
	if len(decrements) == 0 {
		return nil, nil
	}
	companyID := store.CompanyID(ctx)
	...
	rows, err := r.db.Query(ctx, `
		...
		updated AS (
			UPDATE platform_keys pk
			SET gateway_soft_remain = GREATEST(pk.gateway_soft_remain - input.delta, 0),
			    gateway_soft_at = NOW(),
			    gateway_soft_version = pk.gateway_soft_version + 1
			FROM input
			WHERE pk.company_id = $1
			  AND pk.id = input.platform_key_id
			  AND pk.gateway_soft_remain IS NOT NULL
			RETURNING pk.id, pk.key_hash, pk.gateway_soft_remain, pk.gateway_soft_at, pk.gateway_soft_version
		)
		SELECT id, key_hash, gateway_soft_remain, gateway_soft_at, gateway_soft_version
		FROM updated
	...
	if err != nil {
		return nil, fmt.Errorf("decrement gateway soft summaries: %w", err)
	}
	defer rows.Close()

	out := make([]store.GatewaySoftSummary, 0, len(decrements))
	for rows.Next() {
		var item store.GatewaySoftSummary
		var updatedAt *time.Time
		if err := rows.Scan(&item.PlatformKeyID, &item.KeyHash, &item.SoftRemain, &updatedAt, &item.Version); err != nil {
