...
func (r *budgetConsumedRepo) IncrementConsumed(ctx context.Context, axisKind string, axisID uuid.UUID, periodKey string, amount float64) error {
	companyID := store.CompanyID(ctx)
	_, err := r.db.Exec(ctx, `
...
func (r *budgetConsumedRepo) IncrementConsumedBatch(ctx context.Context, deltas []store.ConsumedDelta) error {
	...
	_, err := r.db.Exec(ctx, `
		...
		FROM UNNEST($2::text[], $3::uuid[], $4::text[], $5::float8[])
