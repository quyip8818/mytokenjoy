...
func (s *service) ToggleProviderKey(ctx context.Context, id string, enabled bool) error {
	...
	if parseErr != nil {
		return domain.Validation("invalid provider key id")
	}
	keys, err := s.store.Keys().ProviderKeys(ctx)
	if err != nil {
		return err
	...
	for i := range keys {
		if keys[i].ID == parsedID {
			...
			if s.newAPISync != nil && s.newAPISync.Enabled() {
				if err := s.newAPISync.EnqueueUpsertProviderKey(ctx, parsedID); err != nil {
					return err
				}
			}
			return nil
...
func (s *service) RotateProviderKey(ctx context.Context, id string, newKey string) (types.ProviderKey, error) {
	...
	parsedID, parseErr := uuid.Parse(id)
	if parseErr != nil {
		return types.ProviderKey{}, domain.Validation("invalid provider key id")
	}
	keys, err := s.store.Keys().ProviderKeys(ctx)
	if err != nil {
		return types.ProviderKey{}, err
	...
	for i := range keys {
		if keys[i].ID == parsedID {
			...
			if s.newAPISync != nil && s.newAPISync.Enabled() {
				...
				if err := s.newAPISync.SyncUpsertProviderKey(ctx, parsedID); err != nil {
					return types.ProviderKey{}, domain.ServiceUnavailable("NewAPI Channel sync failed")
				}
			}
			return keys[i], nil
