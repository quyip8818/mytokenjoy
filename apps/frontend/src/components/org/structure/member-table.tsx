...
import {
  useReactTable, getCoreRowModel, flexRender, createColumnHelper, type RowSelectionState,
...
export function MemberTable({
  data, total, page, pageSize, onPageChange,
  onEdit, onStatusChange, onDelete, rowSelection, onRowSelectionChange,
}: MemberTableProps) {
  const columns = useMemo(() => [
    columnHelper.display({
      id: 'select',
      header: ({ table }) => (
        <Checkbox checked={table.getIsAllPageRowsSelected()} onCheckedChange={(c) => table.toggleAllPageRowsSelected(!!c)} />
      ),
      cell: ({ row }) => (
        <Checkbox checked={row.getIsSelected()} onCheckedChange={(c) => row.toggleSelected(!!c)} />
      ),
      size: 40,
    }),
    ...
    columnHelper.accessor('phone', {
      header: '手机号',
      cell: (info) => <span className="tabular-nums text-muted-foreground">{maskPhone(info.getValue())}</span>,
    }),
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="xs" onClick={() => onEdit(member)}>编辑</Button>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {member.status === 'active' ? (
                  <DropdownMenuItem onClick={() => onStatusChange([member.id], 'inactive')}>停用</DropdownMenuItem>
                ) : (
                  <DropdownMenuItem onClick={() => onStatusChange([member.id], 'active')}>启用</DropdownMenuItem>
                )}
                <DropdownMenuItem className="text-destructive" onClick={() => onDelete([member.id])}>删除</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        )
    ...
    <div className="flex flex-1 flex-col overflow-hidden rounded-md border border-border">
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows.map((row) => (
              <TableRow key={row.id} data-state={row.getIsSelected() && 'selected'} className="even:bg-muted/40 hover:bg-muted/50 transition-colors duration-100">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-border px-4 py-3 text-sm text-muted-foreground">
          <span>共 <span className="tabular-nums font-medium text-foreground">{total}</span> 条</span>
          <div className="flex items-center gap-1">
