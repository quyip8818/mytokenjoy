...
import type { AlertRuleView } from '@/lib/budget-alerts'
import { alertRuleFromView } from '@/lib/budget-alerts'
import { groupProjectsByTeam } from '@/lib/budget-alerts'
import {
  Dialog,
  DialogContent,
  DialogFooter,
...
import { DepartmentTreeSelect } from '@/components/budget/department-tree-select'
...
function AlertRuleDialogContent({
  ...
  useEffect(() => {
    void apis.roleApi.list().then(setRoles).catch(() => setRoles([]))
  }, [apis])
  ...
  async function handleSave() {
    ...
    setSaving(true)
    try {
      const payload = alertRuleFromView({
        targetType,
        targetId,
        targetName,
        thresholds,
      ...
      if (rule) {
        await apis.budgetApi.updateAlert(rule.id, payload)
      } else {
