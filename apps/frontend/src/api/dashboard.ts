...
import type { CostSummary, DailyCost, DepartmentCost, ModelUsage, TeamUsage, TopConsumer } from './types'

export const dashboardApi = {
  getCostSummary: (period?: string) =>
    request<CostSummary>(`/dashboard/cost/summary${period ? `?period=${period}` : ''}`),
  getDepartmentCosts: () =>
    request<DepartmentCost[]>('/dashboard/cost/departments'),
  ...
  getModelUsage: () =>
    request<ModelUsage[]>('/dashboard/usage/models'),
  getTeamUsage: () =>
    request<TeamUsage[]>('/dashboard/usage/teams'),
}
