...
function buildDailyCosts(): DailyCost[] {
  const factorSum = DAILY_COST_FACTORS.reduce((sum, f) => sum + f, 0)
  const baseDaily = TOTAL_COST_TARGET / factorSum

  return DAILY_COST_FACTORS.map((factor, i) => {
...
export const mockDailyCosts: DailyCost[] = buildDailyCosts()
...
export const mockTopConsumers: TopConsumer[] = TOP_CONSUMER_SPECS.map((spec) => {
