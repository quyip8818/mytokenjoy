...
test.describe('审计', () => {
  test('operations page has export button', async ({ page }) => {
    ...
    await expect(page.getByTestId('page-audit-operations')).toBeVisible()
    await expect(page.getByRole('button', { name: '导出 CSV' })).toBeVisible()
  })

  test('calls page has export button', async ({ page }) => {
    await page.goto('/audit/calls')
    await expect(page.getByTestId('page-audit-calls')).toBeVisible()
    await expect(page.getByRole('button', { name: '导出 CSV' })).toBeVisible()
  })
